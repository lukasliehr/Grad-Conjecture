...
local instance ngp01RealityCellPeriodPositive : Fact (0 < 2 * Real.pi) :=
  ⟨by positivity⟩

/-- Coefficientwise reality survives the exact phase-weighted equivalence
used by physical evaluation. -/
theorem weightedCoefficientCore_reality
    {dimension grade : ℕ} (parameters : PhaseParameters)
    (field : GradeCore parameters dimension grade)
    (reality : GradeCoreReality parameters field) (cell : ℤ) :
    closedJetConjugate
        ((weightedCoefficientCoreEquiv parameters field.toCore).1 (-cell)) =
      (weightedCoefficientCoreEquiv parameters field.toCore).1 cell := by
  have fixed : gradeCoreConjugation parameters field = field :=
    (gradeCoreConjugation_fixed_iff_reality parameters field).2 reality
  have coefficientReality := congrArg
    (fun value : GradeCore parameters dimension grade => value.toCore.1 cell) fixed
  change closedJetConjugate
      (phaseWeightedJet parameters (-cell) (field.toCore.1 (-cell))) =
    phaseWeightedJet parameters cell (field.toCore.1 cell)
  rw [closedJetConjugate_phaseWeightedJet,
    phaseWeightedJet_neg parameters cell (closedJetConjugate (field.toCore.1 (-cell)))]
  exact congrArg (phaseWeightedJet parameters cell) coefficientReality

theorem cellExponential_conjugate (cell : ℤ) (coordinate : ℝ) :
...
def physicalEvaluationLift {dimension grade : ℕ}
    (parameters : PhaseParameters)
    (field : GradeCore parameters dimension grade)
    (point : ClosedDisk) (cell : ℝ) : ComplexEuclidean dimension :=
  (weightedSmoothEquiv parameters field.toCore).value
    (point, (cell : CellCircle))

theorem physicalEvaluationLift_eq_tsum
    {dimension grade : ℕ} (parameters : PhaseParameters)
    (field : GradeCore parameters dimension grade)
    (point : ClosedDisk) (coordinate : ℝ) :
    physicalEvaluationLift parameters field point coordinate =
      ∑' cell : ℤ,
        cellExponential cell coordinate •
          ((weightedCoefficientCoreEquiv parameters field.toCore).1 cell).value point := by
...
theorem physicalEvaluationLift_periodic
    {dimension grade : ℕ} (parameters : PhaseParameters)
    (field : GradeCore parameters dimension grade)
    (point : ClosedDisk) (cell : ℝ) :
    physicalEvaluationLift parameters field point (cell + 2 * Real.pi) =
      physicalEvaluationLift parameters field point cell := by
  unfold physicalEvaluationLift
  have circleEquality :
...
theorem physicalEvaluationLift_real
    {dimension grade : ℕ} (parameters : PhaseParameters)
    (field : GradeCore parameters dimension grade)
    ...
    cartesianPhysicalConjugation dimension
        (physicalEvaluationLift parameters field point coordinate) =
      physicalEvaluationLift parameters field point coordinate := by
  ...
  rw [physicalEvaluationLift_eq_tsum]
  apply PiLp.ext
  ...
  calc
    (∑' cell : ℤ,
      ...
      have coefficientReality := congrArg
        (fun jet : ClosedJet dimension => jet.value point outputCoordinate)
        (weightedCoefficientCore_reality parameters field reality (-cell))
      simpa only [neg_neg, closedJetConjugate_value_apply] using
