...
namespace Grad.MainAssembly.SampledPhysicalSimilarityRigidity
...
/-- The seed angle seen on the physical circle after integer resampling. -/
def sampledAlphaAngle (period : ℕ) (alpha delta parameter time : ℝ) : ℝ :=
  alphaAngle alpha delta parameter ((period : ℝ) * time)
...
/-- The exact NG_R17 consumer for the integer-resampled seed.  Rescaling the
circle variable by the positive integer period reduces its angle congruence
to the already accepted two-harmonic rigidity theorem. -/
theorem parameter_eq_of_sampled_physicalSimilarity
    ...
    (firstShape : HasPrescribedNormalShape first (period * cellLength) rho
      (sampledAlphaAngle period alpha delta firstParameter))
    (secondShape : HasPrescribedNormalShape second (period * cellLength) rho
      (sampledAlphaAngle period alpha delta secondParameter))
  ...
  have angleCongruence : ∀ time, ∃ integer : ℤ,
      sampledAlphaAngle period alpha delta secondParameter
          ...
          verticalSign * sampledAlphaAngle period alpha delta firstParameter time =
    ...
    exact TensorAngleRigidity.Consumer.angle_congruence_of_signed_tensor_covariance
      rho (sampledAlphaAngle period alpha delta firstParameter time)
      (sampledAlphaAngle period alpha delta secondParameter targetTime)
      verticalSign rhoPositive verticalSignValue normalizedCovariance
  have periodPositive : 0 < period := by
    by_contra notPositive
    have periodZero : period = 0 := Nat.eq_zero_of_not_pos notPositive
    subst period
    simp at axisRadiusPositive
  have periodNonzero : (period : ℝ) ≠ 0 :=
    ne_of_gt (Nat.cast_pos.mpr periodPositive)
  have rescaledCongruence : ∀ time, ∃ integer : ℤ,
      alphaAngle alpha delta secondParameter
            (tangentSign * time + (period : ℝ) * angleShift) -
          verticalSign * alphaAngle alpha delta firstParameter time =
        (integer : ℝ) * Real.pi := by
    intro time
    rcases angleCongruence (time / (period : ℝ)) with ⟨integer, equality⟩
    refine ⟨integer, ?_⟩
  ...
  exact (HarmonicRigidity.Consumer.full_two_parameter_two_sign_harmonic_rigidity
    alpha delta firstParameter secondParameter tangentSign verticalSign
    ((period : ℝ) * angleShift) firstParameterPositive secondParameterPositive
    deltaNonzero alphaNotLattice tangentSignValue verticalSignValue
    rescaledCongruence).2.2.2

end Grad.MainAssembly.SampledPhysicalSimilarityRigidity
