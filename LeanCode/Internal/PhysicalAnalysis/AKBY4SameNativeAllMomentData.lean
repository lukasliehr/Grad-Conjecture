import AKBY3ActualNativeAllPowerEnergy
...
open Set MeasureTheory Grad.PDEBootstrap Grad.GenericCarriers Grad.ClosedJets Grad.CartesianState

/-- SAME primitive native fields with every natural full integer-cell moment. -/
structure OriginalNativeAllMoments (parameters : PhaseParameters)
    (covariantRaw : ℤ → Spatial → PhysicalValue 3) (xiOverRadiusRaw : ℤ → Spatial → PhysicalValue 1) where
  covariant : StartupAllMoments 3
  xiOverRadius : StartupAllMoments 1
...
/-- Genuine original native estimates supply both required startup fields;
no Cartesian H1 or L2 premise is added. -/
theorem actualCartesianFamily_originalNativeAllMoments :
    Nonempty (OriginalNativeAllMoments parameters
  ...
  obtain ⟨covariant,covariantSame⟩ :=
    actualCartesianCovariant_nativeAllCellMoments parameters length compact lengthPositive widthHalf widthLength state small
      source flat fields member sameSources allGrades compatible vanishing graded sameGrade constants estimate
  obtain ⟨xi,xiSame⟩ :=
    actualCartesianXiOverRadius_nativeAllCellMoments parameters length compact lengthPositive widthHalf widthLength state small
      source flat fields member sameSources allGrades compatible vanishing graded sameGrade constants estimate
  exact ⟨⟨covariant,xi,covariantSame,xiSame⟩⟩
