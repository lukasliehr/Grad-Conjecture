...
theorem weightedRotation_projection (weight : ℝ → ℝ) (weightContinuous : Continuous weight)
    (field : SpatialPlane → ComplexEuclidean 1) (continuousField : Continuous field)
    (mode : ℤ) (point : SpatialPlane) :
    angularProjectionValue mode (weightedRotationValue weight field) point =
      weightedRotationValue weight (angularProjectionValue mode field) point := by
  let integrand : ℝ × ℝ → ComplexEuclidean 1 := fun pair =>
  ...
  have leftRow (angle : ℝ) :
      angularCharacter mode angle • weightedRotationValue weight field (planeRotationAction angle point) =
        (2 * Real.pi)⁻¹ • ∫ other in Icc (0 : ℝ) (2 * Real.pi), integrand (angle, other) := by
    unfold weightedRotationValue integrand
  ...
  exact congrArg (fun value : ComplexEuclidean 1 => (2 * Real.pi)⁻¹ • ((2 * Real.pi)⁻¹ • value))
    (integral_integral_swap integrable)

theorem weightedRotationValue_congr_closed (weight : ℝ → ℝ)
    {first second : SpatialPlane → ComplexEuclidean 1}
    (same : ∀ point : ClosedDisk, first point.val = second point.val) (point : ClosedDisk) :
    weightedRotationValue weight first point.val = weightedRotationValue weight second point.val := by
  ...
  simpa only [physicalRotation_eq_orthogonal, planeRotationEquiv_apply, rotatedPoint] using
    congrArg (fun value : ComplexEuclidean 1 => weight angle • value) equality

theorem angularClosedJet_weightedRotation (weight : ℝ → ℝ) (weightSmooth : ContDiff ℝ ∞ weight)
    (mode : ℤ) (field : ClosedJet 1) :
    angularClosedJet mode (weightedRotationJet weight weightSmooth field) =
      weightedRotationJet weight weightSmooth (angularClosedJet mode field) := by
  apply closedJet_eq_of_value_eq
  apply ContinuousMap.ext
  intro point
  have first : ∀ next : ClosedDisk,
      smoothClosedExtension (weightedRotationJet weight weightSmooth field) next.val =
        weightedRotationValue weight (smoothClosedExtension field) next.val :=
    fun next => smoothClosedExtension_value (weightedRotationJet weight weightSmooth field) next
  ...
  change angularProjectionValue mode (smoothClosedExtension (weightedRotationJet weight weightSmooth field)) point.val =
    weightedRotationValue weight (smoothClosedExtension (angularClosedJet mode field)) point.val
  exact (angularProjectionValue_congr_closed mode first point).trans
    ((weightedRotation_projection weight weightSmooth.continuous _ (smoothClosedExtension_smooth field).continuous mode point.val).trans
      (weightedRotationValue_congr_closed weight second point).symm)
