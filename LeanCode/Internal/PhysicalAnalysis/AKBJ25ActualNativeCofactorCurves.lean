...
def cartesianSourceCofactorRows (index : ℕ) : DivisionRow 3 (originalExhaustionRadius length index) :=
  matrixFluxRows parameters (originalCofactorFamily parameters length state.val.val.epsilon state.val.val.field) ((originalCofactorFamily_estimate parameters length state.val.val.rho state.val.val.epsilon state.val.val.field (originalExhaustionPrimitiveRadius_source parameters length compact state.val.val.rho state.val.val.epsilon state.val.val.field small)).actualCoherent) (originalExhaustionRadius length) (originalExhaustionRadius_positive length lengthPositive) (fun index => (originalExhaustionRadius_half length lengthPositive index).trans_lt (by norm_num)) (fun index => cartesianCovariantRow (originalExhaustionRadius length index) (cartesianPolarCovariantFamily parameters length compact lengthPositive widthHalf widthLength state small source flat fields index)) index

def cartesianSourceCofactorCurves (index : ℕ) : SmoothLowPhysicalRow parameters (originalExhaustionRadius length index)
    (originalExhaustionRadius_positive length lengthPositive index) (cartesianSourceCofactorRows parameters length compact lengthPositive widthHalf widthLength state small source flat fields index) :=
  matrixFluxCurves parameters (originalCofactorFamily parameters length state.val.val.epsilon state.val.val.field) ((originalCofactorFamily_estimate parameters length state.val.val.rho state.val.val.epsilon state.val.val.field (originalExhaustionPrimitiveRadius_source parameters length compact state.val.val.rho state.val.val.epsilon state.val.val.field small)).actualCoherent) (originalExhaustionRadius length) (originalExhaustionRadius_positive length lengthPositive) (fun index => (originalExhaustionRadius_half length lengthPositive index).trans_lt (by norm_num)) (fun index => cartesianCovariantRow (originalExhaustionRadius length index) (cartesianPolarCovariantFamily parameters length compact lengthPositive widthHalf widthLength state small source flat fields index)) (fun index => (cartesianSourcePolarCurves parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades index).cartesianCovariant) index

include compatible in
theorem cartesianSourceCofactorRows_compatible (first second : ℕ) (ordered : first ≤ second) :
    originalBulkRestriction 3 (originalExhaustionRadius length second) (originalExhaustionRadius length first)
      (originalExhaustionRadius_antitone length lengthPositive ordered) (cartesianSourceCofactorRows parameters length compact lengthPositive widthHalf widthLength state small source flat fields second) =
      cartesianSourceCofactorRows parameters length compact lengthPositive widthHalf widthLength state small source flat fields first := by
  apply matrixFluxRows_compatible parameters (originalCofactorFamily parameters length state.val.val.epsilon state.val.val.field) ((originalCofactorFamily_estimate parameters length state.val.val.rho state.val.val.epsilon state.val.val.field (originalExhaustionPrimitiveRadius_source parameters length compact state.val.val.rho state.val.val.epsilon state.val.val.field small)).actualCoherent) (originalExhaustionRadius length) (originalExhaustionRadius_positive length lengthPositive) (fun index => (originalExhaustionRadius_half length lengthPositive index).trans_lt (by norm_num))
