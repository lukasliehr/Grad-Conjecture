import AKBT4SameScaledFamilyContinuity
...
theorem actualScaledCovariantRaw_punctured_continuous (ell : ℝ) (ellPositive : 0 < ell) (ellBounded : ell ≤ 1) :
    ContinuousOn (actualScaledCovariantRaw parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades ell) {pair | pair.2 ∈ openUnitDisk \ {(0 : Spatial)}} :=
  scaledNativeFamilyRaw_punctured_continuous parameters (originalExhaustionRadius length) (originalExhaustionRadius_positive length lengthPositive) (fun index => (originalExhaustionRadius_half length lengthPositive index).trans_lt (by norm_num)) (originalExhaustionRadius_tendsto length) (originalExhaustionRadius_antitone length lengthPositive) (fun index => cartesianCovariantRow (originalExhaustionRadius length index) (cartesianPolarCovariantFamily parameters length compact lengthPositive widthHalf widthLength state small source flat fields index)) (fun index => (cartesianSourcePolarCurves parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades index).cartesianCovariant) (cartesianCovariantRows_compatible (originalExhaustionRadius length) (originalExhaustionRadius_antitone length lengthPositive) (cartesianPolarCovariantFamily parameters length compact lengthPositive widthHalf widthLength state small source flat fields) (fun first second ordered => (cartesianSourcePolarAndXi_compatible parameters length compact lengthPositive widthHalf widthLength state small source flat fields compatible first second ordered).1)) ell ellPositive ellBounded

include compatible in
/-- The actual full native gauge product has continuous Fourier cells on
the punctured disk, as needed by the rough current recovery theorem. -/
theorem actualScaledNativeMatrix_cell_continuous {ell : ℝ}
    (admissible : Admissible length parameters.sigma0 parameters.gamma ell) {dimension : ℕ}
    (family : CoefficientFamily length parameters.sigma0 parameters.gamma ell 3 dimension) (cell : ℤ) :
    ContinuousOn (fun point => angularCoefficient (fun angle => startupRawMatrix family
      (actualScaledCovariantRaw parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades ell) (angle,point)) cell) (openUnitDisk \ {(0 : Spatial)}) := by
  apply startupRawCell_punctured_continuous
  apply startupRawMatrix_punctured_continuous admissible
  exact actualScaledCovariantRaw_punctured_continuous parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades compatible ell admissible.2.2.2.1
