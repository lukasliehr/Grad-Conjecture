...
/-- One B10 neighborhood before ell, fields and cell grades makes ALL the
 ...
 Cartesian reduction and full-cell kernel realizations. This is the actual
 coefficient-smallness part of RB startup, not an assumed tensor equation. -/
theorem actualOriginalB10SmallRows (parameters : PhaseParameters)
        ...
        ∀ base : ACore parameters 3,
          ...
          ∃ ledger : ActualLedger parameters admissible rho alpha delta parameter epsilon base,
            ...
            ∃ inverseCoherent : FamilyCoherent (determinantInverseFamily admissible ledger.val.gaugeDeviation),
              ...
              ActualStartupRowsSmall ledger inverseCoherent operatorThreshold := by
  ...
  let rowConstant := startupActualRowsConstant L parameters.sigma0 parameters.gamma four five
  have rowNonnegative : 0 ≤ rowConstant := startupActualRowsConstant_nonnegative _ _ _ _ _
  ...
  have rowsSmall := actualStartupRowsSmall_of_budget ledger four five fourNonnegative fiveNonnegative ledgerBounds
  ...
  exact ⟨ledger, primitiveMargin, inverseCoherent, actualLedgerReduction_of_projection ledger inverseCoherent laws,
    actualStartupKernelRealization ledger inverseCoherent, rowsSmall⟩
