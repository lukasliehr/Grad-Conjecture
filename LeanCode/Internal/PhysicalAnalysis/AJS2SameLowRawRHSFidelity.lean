import AJS1LowOriginalRHSSplit
...
theorem lowRhoPhysicalCoefficient_sub_ae {dimension : ℕ} (parameters : PhaseParameters)
  ...
  rw [ae_all_iff]
  intro mode
  filter_upwards [Lp.coeFn_sub (first mode) (second mode)] with radius subtracted
  change (lowRhoPhysicalWeight parameters lower positive radius mode : ℂ)⁻¹ •
    (first mode - second mode) radius = _
  rw [subtracted]
  simp only [Pi.sub_apply, smul_sub, lowRhoPhysicalCoefficient]

theorem lowRhoPhysicalCoefficient_radius_ae (parameters : PhaseParameters)
    (lower : ℝ) (positive : 0 < lower) (field : DivisionRow 1 lower) :
    ∀ᵐ radius ∂volume.restrict (Icc lower 1), ∀ mode,
      lowRhoPhysicalCoefficient parameters lower positive (radialRadiusRow lower positive field) radius mode =
        radius • lowRhoPhysicalCoefficient parameters lower positive field radius mode := by
  filter_upwards [radialRadiusRow_ae lower positive field] with radius actual
  intro mode
  unfold lowRhoPhysicalCoefficient
  rw [actual mode, smul_comm]
...
theorem sharedStrongLowRawSlope_actualRHS (index : LowAnnularIndex) :
    ∀ᵐ radius ∂volume.restrict (Icc lower 1),
      ...
      let rhs := actualOriginalUnknownRHS parameters length compact lower positive
          ...
          (originalSmoothSourceResponse parameters length compact lower positive lowerHalf lengthPositive
            widthHalf widthLength state small core) radius index.2.val +
        actualOriginalSourceRHS parameters length compact lower positive
          (lowerHalf.trans_lt (by norm_num)) lengthPositive state core radius index.2.val
  ...
  change lowPhysicalRawSlope parameters lower length positive bounded field.ofLp.2
    ...
    let rhs := actualOriginalUnknownRHS parameters length compact lower positive bounded lengthPositive state field radius mode.val +
      actualOriginalSourceRHS parameters length compact lower positive bounded lengthPositive state core radius mode.val
