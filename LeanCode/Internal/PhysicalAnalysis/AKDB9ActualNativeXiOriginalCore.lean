import AKDB8ActualNativeCovariantOriginalCore
...
/-- Actual original Xi, recovered from its own force-determined psi and
its SAME compatible Xi/r row, belongs to the unchanged original core. -/
theorem actualNativeXi_allGraphs_originalCore
    ...
    (regular : ∀ grade, ∃ graph : GraphGrade 1 grade grade openUnitDisk,
      base 1 grade openUnitDisk (fun _ => grade) graph=(data.scaledPsi scale).field) :
    ∃ core : ACore parameters 1,
  ...
  exact originalNativePsiAllOrder_xiCore parameters (originalExhaustionRadius length)
    ...
    scale (data.scaledPsi scale).field regular (data.scaledPsi_same scale)
