import AKBK19ActualForceCellsSmoothness
...
/-- The actual observed native solution satisfies exactly the closed
projected flux equation needed to remove the original axis. -/
theorem actualCartesianPlanarForce_projectedFlux (cell : ℤ) :
    ∀ point : ClosedDisk, 0 < ‖point.val‖ → ‖point.val‖ < 1 →
      closedRadialReflectionValue (fun closed : ClosedDisk => nativePlanarForceSource
        ...
        (actualCartesianForceCorrectionCell parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades cell) closed.val) point =
      closedRadialReflectionValue (fun closed : ClosedDisk => originalPlanarDivergence
        (nativePlanarForceFlux (actualCartesianXiCell parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades cell)
          (actualCartesianCovariantCell parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades cell)) closed.val) point := by
  ...
  apply nativePlanarForce_projected_flux
  ...
  intro radius positive inside angle
  let index := selectedInnerCollar (originalExhaustionRadius length) (originalExhaustionRadius_tendsto length) radius positive
  exact actualCartesianPlanarForceCell_projected parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades compatible cell index radius
    ⟨selectedInnerCollar_lt (originalExhaustionRadius length) (originalExhaustionRadius_tendsto length) radius positive,inside⟩ angle
