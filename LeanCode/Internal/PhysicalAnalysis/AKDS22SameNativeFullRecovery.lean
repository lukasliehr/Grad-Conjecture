...
/-- One actual source solve now carries its native bounds, covariant pure-cell
bound and complete U/S recovery. All choices refer to the SAME family, and
all constants precede the state and source. -/
theorem actualCartesianSource_fullRecovery_quantitative
  ...
  let recovered := actualNative_US_ofCovariant_oneHigh parameters length compact
    lengthPositive widthHalf widthLength state small source flat family.limit
    (fun index => (family.equations index).1) (fun index => (family.equations index).2.1)
    (fun index grade => ⟨family.graded index grade,family.inserted index grade⟩) family.compatible
    1 (by norm_num) stateBound vanishing retainedBound family.graded family.inserted payment gradedBound
    covariant xi covariantSame xiSame
  let vector := recovered.choose
  let scalar := recovered.choose_spec.choose
  have recoveredLaws := recovered.choose_spec.choose_spec
  have vectorSame := recoveredLaws.1
  have scalarSame := recoveredLaws.2.1
  have recovery := recoveredLaws.2.2
  refine ⟨family,nativeBound,baseBound,covariant,vector,xi,scalar,covariantCells,covariantSame,xiSame,scalarSame,vectorSame,recovery,?_⟩
