import AKAW15ActualInsertedCorrectedEnergy
...
/-- All original native inserted grades give finite global energy for the
SAME corrected Cartesian U, with the full coefficient action before cell selection. -/
theorem actualCorrectedWeightedCurve_allGradeEnergy (grade : ℕ) :
  ...
  obtain ⟨C,Cnonnegative,coefficientBound⟩ := correctedCurve_uniformBound parameters length compact state.val
    state.val.val.rho state.val.val.epsilon state.val.val.field lowSmall grade
  ...
  let curves := cartesianCorrectedVectorCurves parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades
  let rows := cartesianCorrectedVectorFamily parameters length compact lengthPositive widthHalf widthLength state small source flat fields
  have coherent := fun first second ordered => (cartesianCorrectedFamilies_compatible parameters length compact lengthPositive widthHalf widthLength state small source flat fields compatible first second ordered).1
  have collarBound (index : ℕ) :
    ...
    have localEnergy := actualObservedPhysicalU_insertedEnergy parameters length compact (originalExhaustionRadius length index)
      ...
      lowSmall source flat (fields index) (member index) (sameSources index) (allGrades index) grade
      (graded index grade) (sameGrade index grade) C Cnonnegative coefficientBound
    ...
    calc
      _ = ∫⁻ radius in Icc (originalExhaustionRadius length index) 1, ENNReal.ofReal (‖(curves index).curve grade radius‖^2) := by
