import AKCX53SameNativeSignedSpatialData
import AKBT24ActualNativeGaugeResolvent
import AKCO18SamePuncturedSignedFirst
import AKCO19SameNativeSignedWeakData
import AKBY5SameExistingScaledAllMoments
import AKCF7SameScaledCovariantAllGraphs
...
/-- Actual native all-order spatial regularity at the unchanged original
phase: all weak equations, source graphs, coefficient identities and
punctured complement graphs and the closed nested induction are derived from the SAME compatible family. -/
theorem actualOriginalNative_allSpatialGrades
    ...
    (outerDisk : tsupport outer ⊆ openUnitDisk) (plateau : Set.EqOn outer (fun _ => 1) (tsupport inside))
    (radial : ∀ first second : Spatial, ‖first‖ = ‖second‖ → inside first = inside second)
    (resolves : ∀ rank, StartupActualRankResolvable admissible ledger.val ledger.property.1 inverseCoherent outer outerSmooth outerCompact rank)
    ...
    ∃ weighted : StartupAllMoments 3,
      ...
      ∀ order, ((StartupSignedFamily.ofNatural weighted length (fixedScale).val).circle).HasSpatialGrade order := by
  ...
  obtain ⟨allNative⟩ := actualCartesianFamily_originalNativeAllMoments parameters length compact lengthPositive widthHalf widthLength state small
    source flat fields member sameSources allGrades compatible vanishing graded sameGrade constants estimate
  have allSameBase : (allNative.scaledCovariant fixedScale).field = weighted.field := by
    apply Lp.ext
  ...
  let allWeighted := (allNative.scaledCovariant fixedScale).withField weighted.field allSameBase
  let signed := StartupSignedFamily.ofNatural allWeighted length (fixedScale).val
  let signedForce := StartupSignedFirstFamily.knownForce parameters source length lengthPositive
  let signedThird := StartupSignedFirstFamily.knownThird parameters source length lengthPositive
  let signedDeterminant := StartupSignedFirstFamily.determinant parameters source length lengthPositive
  have forceBase : signedForce.field = rows.knownForce.field := by
    ...
    rfl
  have thirdBase : signedThird.field = rows.knownThird.field := by
    change (length⁻¹ : ℂ) • base 1 1 openUnitDisk (fun _ => 0)
      (scaledOriginalSourceFirst parameters (source 3) fixedScale) = _
    rw [scaledOriginalSourceFirst_base]
    rfl
  have detBase : signedDeterminant.field = rows.determinant.field := by
    change (((fixedScale).val / length : ℝ) : ℂ) • base 1 1 openUnitDisk (fun _ => 0)
      (scaledOriginalSourceFirst parameters (source 2) fixedScale) = _
    rw [scaledOriginalSourceFirst_base,Complex.ofReal_div]
    rfl
  have scalarFluxSame : originalScalarFluxKernel admissible ledger.val ledger.property.1 inverseCoherent rows.circle.field = rows.scalarFlux.field := by
    change originalScalarMeanFreeKernel (originalValueKernel toroidalPartMap
      (originalMatrixKernel admissible ledger.val.fluxDeviation ledger.property.1.2.2.2.2.1
        (originalCurrentKernel admissible ledger.val.gaugeDeviation ledger.property.1.2.2.2.1 inverseCoherent
          (originalCircleKernel weighted.field)))) = _
    rw [recovered,coefficientFlux]
    rfl
  obtain ⟨signedFlux,_signedFluxBase,signedWeak,signedFluxRegular⟩ := startupNativeRows_sameSignedSpatialData parameters length fixedScale lengthPositive.ne'
    admissible ledger.val ledger.property.1 inverseCoherent rows rawRows
    (((fixedScale).val : ℂ)⁻¹ • startupMomentDilation fixedScale xi.field) phase weak signed.circle rfl
    signedForce signedThird signedDeterminant
    (StartupSignedFirstFamily.knownForce_allSpatialGrade parameters source length lengthPositive)
    (StartupSignedFirstFamily.determinant_allSpatialGrade parameters source length lengthPositive)
    forceBase thirdBase detBase operators.1 operators.2.1 operators.2.2 scalarFluxSame
  let existingNative : OriginalNativeMoments parameters
    ...
    ⟨allNative.covariant.firstThree,allNative.xiOverRadius.firstThree,allNative.covariant_same,allNative.xiOverRadius_same⟩
  obtain ⟨puncturedGraphs,puncturedSame⟩ := Grad.NativePuncturedGraph.actualScaledCovariant_punctured_allGraphs
    parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades compatible
    existingNative fixedScale annular annularCompact annularPunctured annularSmooth
  have reserve (order weight : ℕ) : ∃ graph : GraphGrade 3 order weight openUnitDisk,
      base 3 order openUnitDisk (fun _ => weight) graph = startupCutoffL2 annular annularSmooth annularCompact signed.field := by
    refine ⟨puncturedGraphs order weight,?_⟩
    rw [puncturedSame]
    change startupCutoffL2 annular annularSmooth annularCompact (allNative.scaledCovariant fixedScale).field = _
    rw [allSameBase]
    rfl
  have outside (order : ℕ) := signed.circle_outside_spatialGraph inside annular insideSmooth annularSmooth insideCompact annularCompact
    annularRadial annularPartition order (reserve order)
  refine ⟨allWeighted,weightedSame,?_⟩
  apply startupSame_native_allSpatialGrades parameters lengthPositive.ne' fixedScale admissible ledger.val ledger.property.1 inverseCoherent
    outer inside outerSmooth insideSmooth outerCompact insideCompact plateau
    (Grad.WeightedJets.ZeroExtension.compactLocalizer openUnitDisk (tsupport outer) outerCompact openUnitDisk_isOpen outerDisk)
    radial resolves signed.circle (StartupSignedFirstFamily.knownTensor signedForce signedThird)
    (fun one two order => StartupSignedFirstFamily.knownTensor_allSpatialGrade order signedForce signedThird
      (StartupSignedFirstFamily.knownForce_allSpatialGrade parameters source length lengthPositive order)
      (StartupSignedFirstFamily.knownThird_allSpatialGrade parameters source length lengthPositive order) one two)
    signedFlux signedFluxRegular signedWeak outside
