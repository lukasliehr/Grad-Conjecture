import AKDN43ActualInjectedPhysicalEuler
...
/-- Each actual physical row acting on one prescribed scalar input slot
retains the joint external/coefficient allocation before source payment. -/
theorem actualPhysicalPrimitive_weightedEuler (parameters : PhaseParameters) (length compact lower : ℝ)
    ...
    ‖budget extra • vectorEulerWithinIteratedDerivative (Icc lower 1) rank
      (fun point => radialConjugatedAction parameters lower positive bounded.le
        ...
        power 0 point (hilbertSlotInjection parameters injection (primitive power point))) radius.val‖ ≤
  ...
  obtain ⟨first,second,first0,second0,action⟩ := actualInjectedPhysicalCurve_EulerAllocation parameters length compact lower
    positive bounded row injection power rank
