import AKCU6ConstructedNativeRecoveryConsumer
...
include bounded axis lengthPositive widthHalf widthLength small compactNonnegative alphaSmall deltaSmall parameterSmall radiusPositive radiusBounded

/-- One actual AN3 exhaustion family has all retained grades bounded before
original core recovery. For this SAME family, original U/S recovery alone
supplies the exact two-sided original source realization. -/
theorem actualNativeFamily_originalRecoveryConsumer :
    ...
    (_sameVector : ∀ (point : ClosedDisk), 0<‖point.val‖ → ∀ axial : ℝ,
    ...
    (_sameScalar : ∀ (point : ClosedDisk), 0<‖point.val‖ → ∀ axial : ℝ,
  ...
  let existence := actualNativeFamily_bounded parameters compact lengthPositive widthHalf widthLength
    reference insideR seed insideS base axis small compactNonnegative alphaSmall deltaSmall parameterSmall (flatSource) bounded
  let family := existence.choose
  let constants := existence.choose_spec.choose
  have estimate : ∀ index grade, ‖family.graded index grade‖≤constants grade := existence.choose_spec.choose_spec
  refine ⟨family,constants,estimate,?_⟩
  intro vector sameVector scalar sameScalar
  exact actualNative_fullOriginalSource_twoSided parameters compact lengthPositive widthHalf widthLength
    ...
    radius radiusPositive radiusBounded source family constants estimate vector sameVector scalar sameScalar
