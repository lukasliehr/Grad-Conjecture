import AKBT92MinimalRadiusProbe
...
/-- Reuse the source-matched checked radius construction. -/
abbrev actualNativeStartupRadius := probeMinimalRadius

theorem actualNativeStartupRadius_positive (parameters : PhaseParameters) (length compact : ℝ)
    ...
    0 < actualNativeStartupRadius parameters length compact positive nonnegative outer smooth supported inside :=
  probeMinimalRadius_positive parameters length compact positive nonnegative outer smooth supported inside

theorem originalStartupScale_admissible (parameters : PhaseParameters) (length : ℝ) (positive : 0 < length) :
    Admissible length parameters.sigma0 parameters.gamma (originalStartupScale length positive).val :=
  probeMinimalScale parameters length positive
...
/-- On a single fixed B10 neighborhood, the actual original native solution
has the localized H1 carrier of its SAME weighted covariant quotient. No
weak equation, operator identity, gauge estimate or regularity is assumed. -/
theorem actualOriginalNative_B10_h1
  ...
  exact actualOriginalNative_h1 parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades compatible
