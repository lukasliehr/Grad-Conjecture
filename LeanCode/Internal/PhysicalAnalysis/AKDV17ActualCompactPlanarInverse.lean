import AKDV12ActualTwoCoreNewtonConsumer
...
/-- The actual compact planar estimate reaches the SAME original inverse:
rank-two absorption costs two grades and full-source reconstruction nine. -/
theorem actualPrincipal_oneHigh_of_compactPlanar (shift loss : ℕ) (lossLarge : shift+32≤loss)
    (lowFits : loss+8≤higher)
    (estimate : ActualProductCompactPlanarEstimate
      (actualPrincipalProduct parameters positive reference inside center insideC zeroC outer smooth compact)
      widthHalf widthLength higher higherLarge shift) :
    Nonempty (ActualProductOneHigh
      (actualPrincipalProduct parameters positive reference inside center insideC zeroC outer smooth compact)
      widthHalf widthLength higher (loss+11)) := by
  have covariant := actualPrincipalCovariantOneHigh parameters positive reference inside center insideC zeroC outer smooth compact
    widthHalf widthLength higher higherLarge (loss+2) (by omega) (estimate.toCompactOneHigh loss lossLarge)
  ...
  simpa only [Nat.add_assoc] using covariant.actual_original_oneHigh (by omega) (by omega)
