import AKBM14ActualOriginalFirstForce
...
/-- The SAME physical third force identity, with its original P and L^-1. -/
theorem originalHomogeneous_sevenThirdForce (radius : Icc lower (1:ℝ)) (angles : ℝ×ℝ) :
    ...
    rotated.fullField bounded (radius.val,angles) 2+
      (removePolarMean (fun query => (physicalForceCurves parameters parameters.length compact lower positive bounded state.val 1 covariant).fullField bounded (radius.val,query)) angles) 0-
      (parameters.length:ℂ)⁻¹*seven.fullField bounded (radius.val,angles) 2=0 := by
  dsimp only
  let seven := originalKernelSevenCurves parameters parameters.length state.val.val.rho state.val.val.epsilon state.val.val.field small
    lower positive bounded physicalState.2.1 vector scalar
  let covariant := seven.covariant parameters parameters.length compact lower positive bounded state.val
  ...
  let residual := ((rotated.bulkUnit (0:Fin 1) 2).add force.meanFree).sub ((seven.bulkUnit (0:Fin 1) 2).smul (parameters.length:ℂ)⁻¹)
  have zero : originalCurveNegativeTrace residual radius=0 := by
    ...
    simp only [originalCurveNegativeTrace_sub,originalCurveNegativeTrace_add,originalCurveNegativeTrace_smul,
      originalCurveNegativeTrace_meanFree]
  ...
  simp only [residual,SmoothLowPhysicalRow.fullField_sub _ bounded _ radius.val radius.property angles,
    ...
    samePhysical_fullField_smul _ bounded (parameters.length:ℂ)⁻¹ radius.val radius.property angles,
