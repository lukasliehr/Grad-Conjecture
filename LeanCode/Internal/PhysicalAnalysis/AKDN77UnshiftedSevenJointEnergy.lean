import AKDN76ActualFullSevenEulerEnergy
...
/-- Bounded removal of one auxiliary frequency grade from an actual
stored physical curve, with no change of analytic width. -/
theorem storedSevenCurve_reserve {parameters : PhaseParameters} {lower : ℝ} {positive : 0<lower}
    {row : DivisionRow 7 lower} (seven : SmoothLowPhysicalRow parameters lower positive row)
    (bounded : lower<1) (power : ℕ) (radius : ℝ) (inside : radius∈Icc lower 1) :
    Grad.AnnularWeightedSmoothness.hilbertReserve parameters 7 1 (seven.curve (power+1) radius)=seven.curve power radius := by
  apply lp.ext
  funext mode
  rw [Grad.AnnularWeightedSmoothness.hilbertReserve_apply,seven.shift bounded power 1 radius inside mode]
  simp only [Grad.AnnularWeightedSmoothness.frequencyReserveSymbol,pow_one]
  exact inv_smul_smul₀ (Complex.ofReal_ne_zero.mpr (annularFrequency_pos mode).ne') _

/-- Energy of the original unshifted seven-slot curve. The single
auxiliary grade used to recover the balanced pair is now removed by a
fixed bounded map before applying the covariant reconstruction. -/
theorem nativeSevenCurves_unshiftedSourceEnergy
    ...
    (∫⁻ radius in Icc (originalExhaustionRadius length index) 1, ENNReal.ofReal
      (‖(1+physicalBudget parameters state.val.val.field state.val.val.rho state.val.val.epsilon (10+extra)) •
        vectorEulerWithinIteratedDerivative (Icc (originalExhaustionRadius length index) 1) rank
          ((nativeSevenCurves family index).curve power) radius‖^2)) ≤
  ...
  let result := nativeSevenCurves_originalSourceEnergy parameters length compact lengthPositive widthHalf widthLength index total extra power rank paid cost cost0
  let constant := result.choose
  have constant0 : 0≤constant := result.choose_spec.1
  have bound := result.choose_spec.2
  let mapping := (Grad.AnnularWeightedSmoothness.hilbertReserve parameters 7 1).restrictScalars ℝ
  ...
  have observed := eulerCurve_squareEnergy_observation lower bounded ((nativeSevenCurves family index).curve (power+1)) rank
    (contDiffOn_infty.mp ((nativeSevenCurves family index).smooth (power+1)) rank) mapping weight (constant*payment) energy
  have same := eulerCurve_squareEnergy_congr lower ((nativeSevenCurves family index).curve power)
    (fun point => mapping ((nativeSevenCurves family index).curve (power+1) point))
    (fun radius inside => (storedSevenCurve_reserve (nativeSevenCurves family index) bounded power radius inside).symm) rank weight
