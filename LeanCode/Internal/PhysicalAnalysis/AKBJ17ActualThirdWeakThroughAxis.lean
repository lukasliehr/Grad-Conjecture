import AKBJ16ActualThirdAngularEquation
...
/-- Actual original third equation through the axis for every integer cell and every compact smooth test. All flux and source integrability is obtained from the same native solution. -/
theorem actualOriginalThird_weak (cell : ℤ) :
    ∀ (test : Spatial → ℝ), ContDiff ℝ ∞ test → HasCompactSupport test → tsupport test ⊆ openUnitDisk →
      (∫ point in openUnitDisk, test point • (fun point => (Complex.I * (cell : ℂ)) • actualCartesianXiCell parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades cell point - actualCartesianProjectedThirdForceCell parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades cell point + originalCoreCell parameters (source 3) cell point) point) =
        -(∑ direction : Fin 2, ∫ point in openUnitDisk,
          fderiv ℝ test point (spatialDirection direction) • angularTransportFlux (thirdRotationField length (actualCartesianCovariantCell parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades cell)) direction point) := by
  ...
  exact originalRotation_equation_remove_axis (thirdRotationField length (actualCartesianCovariantCell parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades cell)) rotation.1 rotation.2 (fun point => (Complex.I * (cell : ℂ)) • actualCartesianXiCell parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades cell point - actualCartesianProjectedThirdForceCell parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades cell point + originalCoreCell parameters (source 3) cell point) sourceIntegrable
    (thirdRotationField_smooth length _ _ (sameCovariantCell_punctured_smooth parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades compatible cell))
    (fun point inside => actualOriginalThird_rotation parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades compatible cell point inside)
