import AKDN77UnshiftedSevenJointEnergy
...
/-- One constant for every allocated derivative of the SAME actual stored input. -/
theorem nativeSevenCurves_uniformJointEnergy
    ...
    ∀ extra power rank : ℕ, extra+power+rank≤total →
  ...
  classical
  let Index := {entry : Fin (total+1) × Fin (total+1) × Fin (total+1) //
    entry.1.val+entry.2.1.val+entry.2.2.val≤total}
  let results := fun entry : Index => nativeSevenCurves_unshiftedSourceEnergy parameters length compact lengthPositive
    widthHalf widthLength index total entry.val.1.val entry.val.2.1.val entry.val.2.2.val entry.property cost cost0
  ...
  intro state low small source flat family native extra power rank paid
  let entry : Index := ⟨(⟨extra,by omega⟩,⟨power,by omega⟩,⟨rank,by omega⟩),paid⟩
  have actual := energy entry state low small source flat family native
  apply actual.trans
  ...
  apply (sq_le_sq₀ (mul_nonneg (constants0 entry) payment0) (mul_nonneg constant0 payment0)).mpr
  exact mul_le_mul_of_nonneg_right ((le_abs_self _).trans (uniform entry)) payment0
