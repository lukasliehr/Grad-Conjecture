...
theorem actualOriginalB10RankGaugeResolvent (parameters : PhaseParameters)
  ...
  obtain ⟨lowRadius, lowPositive, lowOne, supplied⟩ := actualOriginalB10AllRankPrincipalGauge parameters L radius
...
/-- Fixed before the native state, source, rank, or signed power. -/
def actualNativeSpatialRadius (parameters : PhaseParameters) (L radius : ℝ)
    (positive : 0 < L) (nonnegative : 0 ≤ radius)
    (outer : Spatial → ℝ) (smooth : ContDiff ℝ ∞ outer) (compact : HasCompactSupport outer)
    (included : tsupport outer ⊆ openUnitDisk) : ℝ :=
  (actualOriginalB10RankGaugeResolvent parameters L radius 1 1 positive nonnegative zero_lt_one zero_lt_one outer smooth compact included).choose

theorem actualNativeSpatialRadius_positive (parameters : PhaseParameters) (L radius : ℝ)
    (positive : 0 < L) (nonnegative : 0 ≤ radius)
    (outer : Spatial → ℝ) (smooth : ContDiff ℝ ∞ outer) (compact : HasCompactSupport outer)
    (included : tsupport outer ⊆ openUnitDisk) :
    0 < actualNativeSpatialRadius parameters L radius positive nonnegative outer smooth compact included :=
  (actualOriginalB10RankGaugeResolvent parameters L radius 1 1 positive nonnegative zero_lt_one zero_lt_one outer smooth compact included).choose_spec.1
