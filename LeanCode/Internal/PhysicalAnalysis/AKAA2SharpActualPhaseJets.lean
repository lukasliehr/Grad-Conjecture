import AKAA1SharpScaledPhaseAllocation
...
theorem startupRatio_derivative_bound {L sigma gamma ell : ℝ}
    ...
    ‖iteratedFDeriv ℝ rank (weightRatio sigma gamma ell (input + shift) input) point.val‖ ≤
      apRatioConstant L sigma gamma rank * scaledCellWeight L ell shift ^ rank *
        scaledCellWeight L ell input ^ (rank - 1) * originalEnvelope sigma gamma ell shift point.val := by
  ...
  have scaledBound := startupScaled_allocation_bound admissible rank input shift
  ...
  calc
    ...
    _ ≤ commonConstant rank *
        ...
        ((1 + gamma) ^ rank * ((rank + 1 : ℝ) * (max 1 L) ^ rank *
          scaledCellWeight L ell shift ^ rank * scaledCellWeight L ell input ^ (rank - 1))) :=
