import AKCA4CriticalOutputEnergyTransfer
...
/-- Existing native inserted coordinates pay any actual bounded output curve. -/
theorem actualObservedOutput_criticalInsertedEnergy {dimension : ℕ} (grade : ℕ) (outputCurve : ℝ → CellL2 dimension)
    ...
    (coefficientBound : ∀ radius, ‖outputCurve radius‖ ≤ constant *
      ‖(actualCartesianSevenCurves parameters length compact lower positive lowerHalf lengthPositive widthHalf widthLength state small coefficientSmall
        source flat point member sameSources allGrades).curve grade radius‖) :
    (∫⁻ radius in Icc lower 1, ENNReal.ofReal
      (‖radius ^ (-(3 / 2 : ℝ)) • outputCurve radius‖ ^ 2)) ≤
  ...
  have energy := criticalCurveEnergy_transfer parameters lower positive packet curves grade gradedPacket packetSame
    outputCurve constant nonnegative
    (fun radius _ => coefficientBound radius)
