import AKDP29ActualCurrentOriginalRankControl
...
theorem compFixed {State : Type*} {parameters : PhaseParameters} {budget : State → ℝ}
    ...
    (operator : StartupSpatialAction rank middle output L ell) (fixed : operator.OriginalRankControlled parameters)
    ...
    (actual : StartupUniformOriginalRank parameters budget kernel ranked) (nonnegative : ∀ state,0≤budget state) :
    StartupUniformOriginalRank parameters budget (fun state => operator.signed.coarse.comp (kernel state))
...
theorem actualRows {State : Type*} (parameters : PhaseParameters) {L ell : ℝ}
    ...
    (currentControlled : StartupUniformOriginalRank parameters
  ...
  have planarCurrent := planar.comp currentControlled (startupMatrixProfileBaseBound L parameters.sigma0 parameters.gamma five)
    ...
    (matrix_rankBound admissible planarEstimate low) nonnegative
  have force := (compFixed (StartupSpatialAction.radial (L := L) (ell := ell) rank)
    (StartupSpatialAction.radial_originalRankControlled parameters rank) planarCurrent nonnegative).smul 2 nonnegative
  ...
  have thirdCurrent := third.comp currentControlled (startupMatrixProfileBaseBound L parameters.sigma0 parameters.gamma five)
    (startupMatrixProfileBaseBound_nonnegative admissible five fiveFixed fiveDeviation)
    (matrix_rankBound admissible thirdEstimate low) nonnegative
  have third := (compFixed (StartupSpatialAction.scalarMeanFree (L := L) (ell := ell) rank)
    (StartupSpatialAction.scalarMeanFree_originalRankControlled parameters rank) thirdCurrent nonnegative).smul 2 nonnegative
  ...
  have flux := flux.comp currentControlled (startupMatrixProfileBaseBound L parameters.sigma0 parameters.gamma four)
    ...
    (matrix_rankBound admissible fluxEstimate low) nonnegative
  have planarFlux := compFixed (StartupSpatialAction.planarMeanFree (L := L) (ell := ell) rank)
    (StartupSpatialAction.planarMeanFree_originalRankControlled parameters rank)
    (compFixed (StartupSpatialAction.value (L := L) (ell := ell) rank planarPartMap) (StartupSpatialAction.value_originalRankControlled parameters rank planarPartMap) flux nonnegative) nonnegative
  have forceAverage := compFixed (StartupSpatialAction.average (L := L) (ell := ell) rank)
    (StartupSpatialAction.average_originalRankControlled parameters rank) force nonnegative
  have forceQuarter := compFixed (StartupSpatialAction.value (L := L) (ell := ell) rank quarterValueMap)
    (StartupSpatialAction.value_originalRankControlled parameters rank quarterValueMap) forceAverage nonnegative
