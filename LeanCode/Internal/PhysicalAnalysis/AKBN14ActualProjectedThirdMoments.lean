...
private def startupThirdHilbertProjection (parameters : PhaseParameters) : CellL2 3 →L[ℂ] CellL2 1 :=
  coefficientOperator parameters 0 (Equiv.refl _) (fun _ => matrixUnit (0 : Fin 1) (2 : Fin 3))
    (norm_nonneg _) (fun _ => le_rfl)

open Grad.GaugeCoefficients.Physical.RadialLedger
/-- The original projected third correction has genuine weighted and raw
joint moments, using the existing completed output-energy bound. -/
theorem actualCartesianProjectedThirdForce_nativeMoments :
    ∃ weighted raw : StartupMoments 1,
      (∀ᵐ point ∂volume.restrict openUnitDisk, ∀ cell : ℤ,
        weighted.field point cell = cartesianWeight parameters cell point • actualCartesianProjectedThirdForceCell parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades cell point) ∧
      (∀ᵐ point ∂volume.restrict openUnitDisk, ∀ cell : ℤ,
        raw.field point cell = actualCartesianProjectedThirdForceCell parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades cell point) := by
  ...
  obtain ⟨weighted,unweighted,weightedSame,unweightedSame,integrable⟩ := actualCartesianFamily_meanFreeOutputMoments parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades
    ...
    selectedCoherent (‖startupThirdHilbertProjection parameters‖ * C) (mul_nonneg (norm_nonneg _) Cnonnegative)
    (fun index radius => by
      change ‖startupThirdHilbertProjection parameters ((cartesianSourceForceMatrixCurves parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades index).curve 2 radius)‖ ≤ _
      ...
      exact ((startupThirdHilbertProjection parameters).le_opNorm _).trans
        ((mul_le_mul_of_nonneg_left matrixBound (norm_nonneg _)).trans_eq (mul_assoc _ _ _).symm))

  let factor := (-2 * (length : ℂ))
  let mapping : ComplexEuclidean 1 →L[ℂ] ComplexEuclidean 1 := factor • ContinuousLinearMap.id ℂ _
  let weightedFinal := weighted.map (originalValueKernel mapping) (originalValueKernel_cellwise mapping)
  let rawFinal := unweighted.map (originalValueKernel mapping) (originalValueKernel_cellwise mapping)
  have rawSame := Grad.ActualForceMoments.startupValueMap_same unweighted mapping _ unweightedSame
  have weightedSameFinal := Grad.ActualForceMoments.startupValueMap_same weighted mapping _ weightedSame
  refine ⟨weightedFinal,rawFinal,?_,?_⟩
  · filter_upwards [weightedSameFinal] with point same
    intro cell
