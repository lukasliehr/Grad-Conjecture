...
open Grad.PDEBootstrap Grad.ActualCartesianWeakEquations
include compatible

/-- SAME signed cofactor convolution, prior to taking the integer cell. -/
theorem actualCofactorCell_local (cell : ℤ) (index : ℕ) (point : SpatialPlane)
    ...
    actualCartesianCofactorFluxCell parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades cell point =
      angularCoefficient (fun axial => (cartesianSourceCofactorCurves parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades index).cartesianField
        ((originalExhaustionRadius_half length lengthPositive index).trans_lt (by norm_num)) (point,axial)) cell := by
  exact nativeFamilyCell_local parameters (originalExhaustionRadius length) (originalExhaustionRadius_positive length lengthPositive) (fun index => (originalExhaustionRadius_half length lengthPositive index).trans_lt (by norm_num)) (originalExhaustionRadius_tendsto length) (originalExhaustionRadius_antitone length lengthPositive) (cartesianSourceCofactorRows parameters length compact lengthPositive widthHalf widthLength state small source flat fields) (cartesianSourceCofactorCurves parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades) (cartesianSourceCofactorRows_compatible parameters length compact lengthPositive widthHalf widthLength state small source flat fields compatible) cell index point inside

/-- Genuine derivatives of the full cofactor convolution followed by cell selection. -/
theorem actualCofactorCell_fderiv_local (cell : ℤ) (index : ℕ) (point : SpatialPlane)
    (inside : ‖point‖ ∈ Ioo (originalExhaustionRadius length index) 1) (direction : SpatialPlane) :
    fderiv ℝ (actualCartesianCofactorFluxCell parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades cell) point direction =
...
/-- The actual cofactor cell is smooth on every punctured disk point. -/
theorem actualCofactorCell_smooth (cell : ℤ) :
    ContDiffOn ℝ ∞ (actualCartesianCofactorFluxCell parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades cell) (openUnitDisk \ {(0 : SpatialPlane)}) := by
  intro point inside
  exact (gluedCartesianCellField_smoothAt parameters (originalExhaustionRadius length) (originalExhaustionRadius_positive length lengthPositive) (fun index => (originalExhaustionRadius_half length lengthPositive index).trans_lt (by norm_num)) (originalExhaustionRadius_tendsto length) (originalExhaustionRadius_antitone length lengthPositive) (cartesianSourceCofactorRows parameters length compact lengthPositive widthHalf widthLength state small source flat fields) (cartesianSourceCofactorCurves parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades) (cartesianSourceCofactorRows_compatible parameters length compact lengthPositive widthHalf widthLength state small source flat fields compatible) cell point
