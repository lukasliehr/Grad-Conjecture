...
/-- Typed smallness for this SAME actual native state. -/
theorem actualNativeState_small :
    physicalBudget parameters (nativeState).val.val.field (nativeState).val.val.rho (nativeState).val.val.epsilon 8≤
      originalExhaustionPrimitiveRadius parameters parameters.length compact := by
  change physicalBudget parameters (actualFiniteCurrentField parameters reference insideR seed insideS base)
    (seed 0) base.1 8≤originalExhaustionPrimitiveRadius parameters parameters.length compact
  exact actualJetExhaustion_exSmall parameters parameters.length compact reference insideR seed insideS base small
...
/-- The actual finite-jet residual has a reference-domain correction from its
SAME native U/S; no residual-forward identity is supplied as a premise. -/
theorem actualNativeResidual_referenceSolution
 ...
 :
    ∃ correction : stateSmoothRange parameters reference insideR,
      (literalPhysicalSmoothForward parameters parameters.length reference insideR seed insideS base axis correction).val=residual := by
  have sourceMember : residual∈sourceSmoothRange parameters := by
    rw [actualFiniteSourceResidual_literal parameters parameters.length (seed 0) lengthPositive reference insideR seed insideS base currentLow axis source]
    exact (source.val-literalPhysicalSmoothForward parameters parameters.length reference insideR seed insideS base axis
      (actualFiniteReferenceLift parameters parameters.length (seed 0) lengthPositive reference insideR seed insideS base currentLow source)).property
  have solution := nativeFamily_referenceSolution parameters compact lengthPositive widthHalf widthLength nativeState nativeSmall residual residualFlat
    family vector sameVector
    (actualFiniteSourceResidual_higherVanishing parameters parameters.length (seed 0) lengthPositive reference insideR seed insideS base currentLow axis source)
    constants estimate scalar sameScalar reference insideR seed insideS base axis
    (actualJetExhaustionState_originalSeed parameters compact reference insideR seed insideS base small compactNonnegative alphaSmall deltaSmall parameterSmall)
    (actualJetExhaustionState_samePhysicalBase parameters compact reference insideR seed insideS base small compactNonnegative alphaSmall deltaSmall parameterSmall)
    (actualJetExhaustionState_samePhysicalEpsilon parameters compact reference insideR seed insideS base small compactNonnegative alphaSmall deltaSmall parameterSmall)
    sourceMember
  exact ⟨solution.choose,solution.choose_spec.1⟩
