...
include same in
 theorem actualRecoveredVector_polar (index : ℕ) (radius : ℝ)
    (inside : radius ∈ Icc (originalExhaustionRadius length index) 1) (polar axial : ℝ) :
    coreValue core (Grad.SourceCollarDivision.polarClosedPoint radius polar
      ((originalExhaustionRadius_positive length lengthPositive index).le.trans inside.1) inside.2) axial =
      (cartesianCorrectedVectorCurves parameters length compact lengthPositive widthHalf widthLength state small source flat
  ...
  have pointNorm : ‖(Grad.SourceCollarDivision.polarClosedPoint radius polar positive.le inside.2).val‖=radius :=
    (polarPlane_norm radius polar).trans (abs_of_pos positive)
  rw [coreValue_originalPhysical,same _ (by rw [pointNorm]; exact positive),
    actualCartesianVectorField_same parameters length compact lengthPositive widthHalf widthLength state small source flat fields
      member sameSources allGrades compatible index _ (by change ‖(Grad.SourceCollarDivision.polarClosedPoint radius polar positive.le inside.2).val‖∈_; rw [pointNorm]; exact inside)]
  simpa only [polarPlane_originalParametrization,Grad.SourceCollarDivision.polarClosedPoint] using
    (cartesianCorrectedVectorCurves parameters length compact lengthPositive widthHalf widthLength state small source flat
