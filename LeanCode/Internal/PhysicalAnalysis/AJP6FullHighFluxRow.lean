...
private theorem highFlux_source_split (length radius : ℝ) (mode : ℤ × ℤ)
...
theorem originalSmoothHighXRHS_actual (mode : HighAnnularMode) :
    originalSmoothHighXRHS parameters length compact lower positive lowerHalf lengthPositive widthHalf widthLength state small core mode
  ...
  have literal : originalSmoothHighXRHS parameters length compact lower positive lowerHalf lengthPositive widthHalf widthLength state small core mode =
  ...
  exact highFlux_source_split length radius mode.val _ _ _ _ _ _ _ _ _
