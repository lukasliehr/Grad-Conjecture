import AKDS14ActualNativeReferenceNorm
...
theorem actualNativeResidual_referenceSolution_quantitative
 ...
 :
    ∃ correction : stateSmoothRange parameters reference insideR,
      (literalPhysicalSmoothForward parameters parameters.length reference insideR seed insideS base axis correction).val=residual ∧
      ∀ grade, ‖Grad.SmoothingFamily.stateToGrade parameters grade correction.val‖ ≤
  ...
  have solution := nativeFamily_referenceSolution_quantitative parameters compact lengthPositive widthHalf widthLength nativeState nativeSmall residual residualFlat
    ...
    sourceMember seedPatch compactPatch insidePatch seedIn
  exact solution
