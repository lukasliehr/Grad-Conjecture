import AKDS22SameNativeFullRecovery
import AKDM9ActualJetCovariantCellTwenty
...
/-- The actual finite-jet residual carries all SAME original physical cores,
native/pure-cell twenty-loss bounds, the sharper SAME residual native eight-loss bound,
and complete U/S recovery together. All constants precede state and source. -/
theorem actualFiniteJet_fullRecovery_withResidualNative
    (parameters : PhaseParameters) (length compact : ℝ) (lengthPositive : 0 < length) :
    ∃ residualNativeConstant : ℕ → ℝ, (∀ t, 0 ≤ residualNativeConstant t) ∧
    ...
    ∃ family : NativeCartesianFamily parameters length compact lengthPositive widthHalf widthLength state exSmall residual flat,
      (∀ index grade, family.nativeNorm index grade ≤ residualNativeConstant grade *
        (‖quotientEta parameters (grade+8) residual‖+
          physicalBudget parameters state.val.val.field state.val.val.rho state.val.val.epsilon (grade+14)*‖quotientEta parameters 8 residual‖)) ∧
    ...
    ∃ covariant vector : ACore parameters 3, ∃ xi scalar : ACore parameters 1,
      ...
      (∀ (point : ClosedDisk), 0 < ‖point.val‖ → ∀ axial : ℝ,
        (originalPhysicalClosedJet parameters vector).value (point,(axial : CellCircle))=
          actualCartesianVectorField parameters length compact lengthPositive widthHalf widthLength state exSmall residual flat family.limit (fun index => (family.equations index).1) (fun index => (family.equations index).2.1)
            (fun index grade => ⟨family.graded index grade,family.inserted index grade⟩) (point.val,axial)) ∧
  ...
  let certificate := actualCartesianSource_fullRecovery_quantitative parameters length compact lengthPositive
  ...
  refine ⟨nativeCost, nativeNonnegative, (fun grade => nativeCost grade*paymentCost grade),
  ...
  let vector := coreResult.choose_spec.choose
  let xi := coreResult.choose_spec.choose_spec.choose
  let scalar := coreResult.choose_spec.choose_spec.choose_spec.choose
  have identities := coreResult.choose_spec.choose_spec.choose_spec.choose_spec
  ...
  have vectorSame := identities.2.2.2.2.1
  have recovery := identities.2.2.2.2.2.1
  have cellBound := identities.2.2.2.2.2.2
  ...
  refine ⟨family,native,?_,?_,covariant,vector,xi,scalar,covariantCells,covariantSame,xiSame,scalarSame,vectorSame,recovery,?_⟩
  · intro index grade
