import AKDB7SameNativeOriginalCoreConsumer
...
/-- The original compatible native covariant is one original-width ACore
once the actual whole-disk induction supplies its SAME weighted graphs. -/
theorem actualNativeCovariant_allGraphs_originalCore
    (scale : Scale) (field : StartupL2 3)
    (regular : ∀ grade, ∃ graph : GraphGrade 3 grade grade openUnitDisk,
      base 3 grade openUnitDisk (fun _ => grade) graph=field)
    (same : ∀ᵐ point ∂volume.restrict openUnitDisk, ∀ cell : ℤ,
      field point cell=physicalWeight parameters.sigma0 parameters.gamma scale.val cell point •
        actualCartesianCovariantCell parameters length compact lengthPositive widthHalf widthLength state small source flat
          fields member sameSources allGrades cell (scale.val • point)) :
    ∃ core : ACore parameters 3,
      (∀ (point : ClosedDisk), 0 < ‖point.val‖ → ∀ cell : ℤ,
        (core.val cell).value point=actualCartesianCovariantCell parameters length compact lengthPositive widthHalf widthLength
          state small source flat fields member sameSources allGrades cell point.val) ∧
  ...
  exact originalNativeAllOrder_physicalCore parameters (originalExhaustionRadius length)
