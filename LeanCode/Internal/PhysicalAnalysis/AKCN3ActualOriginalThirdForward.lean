import AKCN2ActualRecoveredThirdPolar
...
/-- SAME recovered original cores satisfy the literal sourced fourth forward
row throughout the closed disk, including the axis and outer boundary. -/
theorem actualRecoveredScalar_third :
    quotientRowsDerivative parameters length 1 physicalState ![(0,core,scalar)] 3=source 3 := by
  ...
  have equation := actualRecoveredScalar_third_polar parameters length compact lengthPositive widthHalf widthLength state small
    source flat fields member sameSources allGrades compatible core same scalar sameScalar physicalState sameBase sameEpsilon index ‖point.val‖ inside (angle,axial)
  ...
  change coreValue (quotientRowsDerivative parameters length 1 physicalState ![(0,core,scalar)] 3)
    ...
    coreValue (source 3) (Grad.SourceCollarDivision.polarClosedPoint ‖point.val‖ angle
