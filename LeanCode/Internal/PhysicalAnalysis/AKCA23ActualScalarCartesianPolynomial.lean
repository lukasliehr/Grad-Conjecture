...
open Grad.ActualCartesianWeakEquations Grad.ActualScalarWeakEquations
 ...
 theorem actualNativeXiField_polar (index : ℕ) (radius : ℝ)
    ...
    actualCartesianXiFamilyField parameters length compact lengthPositive widthHalf widthLength state small source flat
      ...
      radius • (cartesianSourceXiOverRadiusCurves parameters length compact lengthPositive widthHalf widthLength state small source flat
  ...
  rw [gluedCartesianFamilyField_same parameters (originalExhaustionRadius length) (originalExhaustionRadius_positive length lengthPositive)
    ...
    index _ (by change ‖polarPlane (radius,polar)‖∈_; rw [pointNorm]; exact inside)]
  rw [pointNorm]
  congr 1
  simpa only [polarPlane_originalParametrization] using
    (cartesianSourceXiOverRadiusCurves parameters length compact lengthPositive widthHalf widthLength state small source flat
 ...
 theorem actualNativeCovariantField_polar (index : ℕ) (radius : ℝ)
    (inside : radius ∈ Icc (originalExhaustionRadius length index) 1) (polar axial : ℝ) :
    actualCartesianCovariantFamilyField parameters length compact lengthPositive widthHalf widthLength state small source flat
      fields member sameSources allGrades (polarPlane (radius,polar),axial)=
      cartesianCovariantValue polar ((cartesianSourcePolarCurves parameters length compact lengthPositive widthHalf widthLength state small source flat
        fields member sameSources allGrades index).fullField
        ((originalExhaustionRadius_half length lengthPositive index).trans_lt (by norm_num)) (radius,polar,axial)) := by
  have positive : 0 < radius := (originalExhaustionRadius_positive length lengthPositive index).trans_le inside.1
  have pointNorm : ‖polarPlane (radius,polar)‖=radius := (polarPlane_norm radius polar).trans (abs_of_pos positive)
  unfold actualCartesianCovariantFamilyField
  rw [gluedCartesianFamilyField_same parameters (originalExhaustionRadius length) (originalExhaustionRadius_positive length lengthPositive)
    (fun index => (originalExhaustionRadius_half length lengthPositive index).trans_lt (by norm_num))
    (originalExhaustionRadius_tendsto length) (originalExhaustionRadius_antitone length lengthPositive) _ _
    ...
    index _ (by change ‖polarPlane (radius,polar)‖∈_; rw [pointNorm]; exact inside)]
  have toPolar := (cartesianSourcePolarCurves parameters length compact lengthPositive widthHalf widthLength state small source flat
    fields member sameSources allGrades index).cartesianCovariant.cartesianField_polar
      ((originalExhaustionRadius_half length lengthPositive index).trans_lt (by norm_num)) radius positive polar axial
  rw [show (cartesianSourcePolarCurves parameters length compact lengthPositive widthHalf widthLength state small source flat
    fields member sameSources allGrades index).cartesianCovariant.cartesianField
      ((originalExhaustionRadius_half length lengthPositive index).trans_lt (by norm_num)) (polarPlane (radius,polar),axial)=_ from
      by simpa only [polarPlane_originalParametrization] using toPolar]
  exact SmoothLowPhysicalRow.fullField_cartesianCovariant _ _ radius inside (polar,axial)

/-- The SAME literal original scalar is Xi plus the polar mean-free polynomial `(Jy) · a_C`.
The Cartesian covariant here is the full native field, before any cell coefficient is taken. -/
 theorem actualNativeScalarField_cartesianPolynomial (radius : ℝ) (positive : 0 < radius) (bounded : radius≤1) (polar axial : ℝ) :
    actualNativeScalarField parameters length compact lengthPositive widthHalf widthLength state small source flat
      ...
      actualCartesianXiFamilyField parameters length compact lengthPositive widthHalf widthLength state small source flat
        ...
        removePolarMean (fun query => scalarTangentialPolynomial (polarPlane (radius,query.1))
          (actualCartesianCovariantFamilyField parameters length compact lengthPositive widthHalf widthLength state small source flat
            fields member sameSources allGrades (polarPlane (radius,query.1),query.2))) (polar,axial) := by
  ...
  have inside : radius ∈ Icc (originalExhaustionRadius length index) 1 :=
    ⟨(selectedInnerCollar_lt (originalExhaustionRadius length) (originalExhaustionRadius_tendsto length) radius positive).le,bounded⟩
  simp_rw [actualNativeScalarField_polar parameters length compact lengthPositive widthHalf widthLength state small source flat
    ...
    actualNativeXiField_polar parameters length compact lengthPositive widthHalf widthLength state small source flat
    fields member sameSources allGrades compatible index radius inside,
    actualNativeCovariantField_polar parameters length compact lengthPositive widthHalf widthLength state small source flat
    fields member sameSources allGrades compatible index radius inside]
  exact correctedScalar_cartesianPolynomial
    (cartesianSourceXiOverRadiusCurves parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades index)
    (cartesianSourcePolarCurves parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades index)
    _ radius inside (polar,axial)
