...
/-- Each actual physical row applied to a prescribed original input slot
has the joint source energy at original width, with independent F4. -/
theorem actualPhysicalPrimitive_jointEnergy (parameters : PhaseParameters) (length compact lower : ℝ)
    (positive : 0 < lower) (bounded : lower < 1) (total extra power rank : ℕ)
    (paid : extra+power+rank ≤ total) (row : Fin 3) (injection : Fin 7) (slot : Fin 4) :
      ...
      (‖(1+physicalBudget parameters state.val.val.field state.val.val.rho state.val.val.epsilon (10+extra)) •
        vectorEulerWithinIteratedDerivative (Icc lower 1) rank
          (fun point => radialConjugatedAction parameters lower positive bounded.le
            ...
            power 0 point (hilbertSlotInjection parameters injection (actualCartesianPrimitiveCurve parameters length lower positive bounded source slot power point))) radius‖^2)) ≤
  ...
  obtain ⟨actionConstant,action0,actionBound⟩ := actualPhysicalPrimitive_weightedEuler parameters length compact lower
    positive bounded total extra power rank paid row injection slot
  ...
  have actual := dominated_squareEnergy (volume.restrict (Icc lower 1))
    (fun radius => budget extra • vectorEulerWithinIteratedDerivative (Icc lower 1) rank
      (fun point => radialConjugatedAction parameters lower positive bounded.le
        (lowPhysicalRowKernel parameters length compact state row)
        power 0 point (hilbertSlotInjection parameters injection (primitive power point))) radius)
