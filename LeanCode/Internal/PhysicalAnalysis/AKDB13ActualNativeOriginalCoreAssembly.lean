...
import AKCX60SameNativeAllOrderConsumer
...
open Grad.OriginalCoreRealization Grad.ActualCartesianWeakEquations

/-- Actual native covariant, Xi and corrected scalar are genuine original
cores at one fixed B10 radius. Every graph, weak equation, phase identity
and polynomial recovery is proved for the SAME compatible native family. -/
theorem actualNative_covariantXiScalar_cores
    ...
    ∃ covariant : ACore parameters 3, ∃ xi scalar : ACore parameters 1,
      (∀ (point : ClosedDisk), 0 < ‖point.val‖ → ∀ cell : ℤ,
        (covariant.val cell).value point=actualCartesianCovariantCell parameters length compact lengthPositive widthHalf widthLength state small
          source flat fields member sameSources allGrades cell point.val) ∧
  ...
  obtain ⟨weighted,weightedSame,weightedRegular⟩ := actualOriginalNative_allOrders parameters length compact lengthPositive
    widthHalf widthLength state small source flat fields member sameSources allGrades compatible
    M Mnonnegative stateBound vanishing nativeBound graded sameGrade constants estimate low
  ...
  have sameField : weighted.field=(allNative.scaledCovariant fixedScale).field := by
    apply Lp.ext
    filter_upwards [weightedSame,allNative.scaledCovariant_same fixedScale] with point one two
    apply lp.ext
    funext cell
    exact (one cell).trans (two cell).symm
  have circleRegular (order : ℕ) :
      ((StartupSignedFamily.ofNatural (allNative.scaledCovariant fixedScale) length (fixedScale).val).circle).HasSpatialGrade order :=
    (weightedRegular order).congr (congrArg originalCircleKernel sameField)
  let admissible := originalStartupScale_admissible parameters length lengthPositive
  have provider := (Classical.choose_spec (actualOriginalB10RankGaugeResolvent parameters length compact 1 1
    lengthPositive state.val.val.compactNonnegative zero_lt_one zero_lt_one actualNativeOuterCutoff
    actualNativeOuterCutoff_smooth actualNativeOuterCutoff_compact actualNativeOuterCutoff_inside)).2.2
  obtain ⟨ledger,_primitive,inverseCoherent,_reduction,_rows,_resolves,
  ...
  obtain ⟨fullRegular,scalarRegular⟩ := actualNative_covariantPsi_allSpatialGraphs parameters length compact lengthPositive widthHalf widthLength
    state small source flat fields member sameSources allGrades compatible M Mnonnegative stateBound vanishing nativeBound
    graded sameGrade constants estimate admissible ledger inverseCoherent gaugeConstants gaugeNonnegative gaugeLow gaugeBound determinantSmall
    allNative circleRegular
  have covariantRegular (grade : ℕ) : ∃ graph : GraphGrade 3 grade grade openUnitDisk,
      base 3 grade openUnitDisk (fun _ => grade) graph=(allNative.scaledCovariant fixedScale).field := by
    obtain ⟨graph,same⟩ := fullRegular grade 0 grade
    rw [StartupSignedFamily.zero] at same
    exact ⟨graph,same⟩
  obtain ⟨covariant,covariantCells,covariantSame⟩ := actualNativeCovariant_allGraphs_originalCore parameters length compact lengthPositive
    widthHalf widthLength state small source flat fields member sameSources allGrades compatible fixedScale
    (allNative.scaledCovariant fixedScale).field covariantRegular (allNative.scaledCovariant_same fixedScale)
  obtain ⟨xi,xiSame⟩ := actualNativeXi_allGraphs_originalCore parameters length compact lengthPositive widthHalf widthLength state small source flat
    fields member sameSources allGrades compatible allNative fixedScale (fun grade => scalarRegular grade grade)
  refine ⟨covariant,xi,recoveredScalarOriginalCore parameters covariant xi,covariantCells,covariantSame,xiSame,?_⟩
  exact actualNativeScalar_fromCovariantXi_originalCore parameters length compact lengthPositive widthHalf widthLength state small source flat
    fields member sameSources allGrades compatible covariant xi covariantSame xiSame
