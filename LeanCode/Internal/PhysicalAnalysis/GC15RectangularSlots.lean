...
/- The accepted GC12 slot-product proof with its scalar norm argument unchanged,
but retaining independent input, intermediate and output dimensions. -/
...
theorem slotWeightedCompositionTerm_apply {L sigma gamma ell : ℝ}
  ...
  simpa only [smul_smul, mul_assoc, mul_left_comm, mul_comm] using congrArg
    (fun value : OperatorValue inputDimension outputDimension =>
...
theorem slotWeightedCompositionTerm_norm_le {L sigma gamma ell : ℝ}
    (admissible : Admissible L sigma gamma ell) (grade : ℕ) {inputDimension middleDimension outputDimension : ℕ}
    (outer : Coefficient L sigma gamma ell grade middleDimension outputDimension)
    (inner : Coefficient L sigma gamma ell grade inputDimension middleDimension)
    (cell first : ℤ) (slot : RegularitySlot grade)
    (physical : DerivativeSplit (slotDerivativeIndex slot)) :
...
theorem slotWeightedCompositionTerm_tsum_norm_le {L sigma gamma ell : ℝ}
    ...
    ‖∑' first : ℤ,
      ...
      ∑' first : ℤ,
        ‖slotWeightedCompositionTerm L sigma gamma ell grade outer inner cell first
          slot physical‖ := by
  let term : ℤ → ContinuousMap ClosedDisk (OperatorValue inputDimension outputDimension) :=
    fun first =>
      slotWeightedCompositionTerm L sigma gamma ell grade outer inner cell first
        slot physical
