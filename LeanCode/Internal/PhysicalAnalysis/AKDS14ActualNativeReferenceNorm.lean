import AKDS13ActualReferencePairNorm
...
/-- The existing exact reference solution also retains its same-grade
norm payment, selected uniformly before the native state and source. -/
theorem nativeFamily_referenceSolution_quantitative :
  ...
  obtain ⟨correction,forward,same⟩ := nativeFamily_referenceSolution parameters compact lengthPositive widthHalf widthLength
    state small source flat family vector sameVector vanishing constants estimate scalar sameScalar reference insideR
    seed insideS base axis sameSeed sameBase sameEpsilon sourceMember
  refine ⟨correction,forward,?_⟩
  intro grade
  exact (actualReferencePair_bound_on_patch parameters grade reference insideR seedPatch compactPatch insidePatch).choose_spec.2
    seed seedIn insideS correction vector scalar same
