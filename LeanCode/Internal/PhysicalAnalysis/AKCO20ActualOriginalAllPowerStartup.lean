...
/-- Actual native all-power first regularity at the unchanged original
phase: all weak equations, source graphs, coefficient identities and
punctured complement graphs are derived from the SAME compatible family. -/
theorem actualOriginalNative_allSignedPowers_first
  ...
  have scalarFluxSame : originalScalarFluxKernel admissible ledger.val ledger.property.1 inverseCoherent rows.circle.field = rows.scalarFlux.field := by
    ...
    rfl
  obtain ⟨signedFlux,_signedFluxBase,signedWeak⟩ := startupNativeRows_sameSignedWeakData parameters length fixedScale
    ...
    signedForce signedThird signedDeterminant forceBase thirdBase detBase operators.1 operators.2.1 operators.2.2 scalarFluxSame
  ...
  have reserve (weight : ℕ) : ∃ graph : GraphGrade 3 1 weight openUnitDisk,
      base 3 1 openUnitDisk (fun _ => weight) graph = startupCutoffL2 annular annularSmooth annularCompact signed.field := by
    refine ⟨puncturedGraphs 1 weight,?_⟩
    rw [puncturedSame]
    change startupCutoffL2 annular annularSmooth annularCompact (allNative.scaledCovariant fixedScale).field = _
    rw [allSameBase]
    rfl
  have outside := signed.circle_outside_first inside annular insideSmooth annularSmooth insideCompact annularCompact
    annularRadial annularPartition reserve
  refine ⟨allWeighted,weightedSame,?_⟩
  exact startupSame_allSignedPowers_first parameters length lengthPositive.ne' fixedScale admissible ledger.val ledger.property.1 inverseCoherent
    ...
    signed.circle (StartupSignedFirstFamily.knownTensor signedForce signedThird) signedFlux signedWeak outside
