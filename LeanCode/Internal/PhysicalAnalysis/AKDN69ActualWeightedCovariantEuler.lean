import AKDN68ActualCovariantEulerAllocation
import AKDN44ActualWeightedPhysicalPrimitive
...
open Grad.AnnularCurrentLow Grad.BoundaryKernelAction

/-- External and internal coefficient weights are combined before
integrating the actual covariant reconstruction. -/
theorem actualEulerFamily_weightedEuler {source target : ℕ}
    ...
    ‖budget extra • vectorEulerWithinIteratedDerivative (Icc lower 1) rank
      (fun point => radialConjugatedAction parameters lower positive bounded.le
        (kernel state.val)
        power 0 point (input power point)) radius.val‖ ≤
      constant*eulerAllocationSum (fun order inputRank =>
        ‖budget (extra+order) • vectorEulerWithinIteratedDerivative (Icc lower 1) inputRank (input power) radius.val‖+
        ‖budget (extra+order+power) • vectorEulerWithinIteratedDerivative (Icc lower 1) inputRank (input 0) radius.val‖)
        (eulerLeibnizTerms rank) := by
  obtain ⟨first,second,first0,second0,action⟩ := actualEulerFamily_curveAllocation parameters length compact lower
    positive bounded kernel family smooth power rank
  ...
  intro state unit input inputSmooth inputSame radius inside
  dsimp only
  let budget := fun order => 1+physicalBudget parameters state.val.val.field state.val.val.rho state.val.val.epsilon (10+order)
  have budget0 (order : ℕ) : 0 ≤ budget order := add_nonneg zero_le_one (physicalBudget_nonnegative _ _ _ _ _)
  have actual := mul_le_mul_of_nonneg_left (action state unit input inputSmooth inputSame radius inside) (budget0 extra)
  ...
  let high := vectorEulerWithinIteratedDerivative (Icc lower 1) term.2 (input power) radius.val
  let low := vectorEulerWithinIteratedDerivative (Icc lower 1) term.2 (input 0) radius.val
