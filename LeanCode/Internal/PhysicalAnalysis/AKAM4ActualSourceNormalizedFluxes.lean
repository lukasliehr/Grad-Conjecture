import AKAM3SameSourceNativeFluxes
...
/-- The SAME actual original source supplies determinant flux, Cartesian covariant,
and Xi, with the full-disk bounds needed by normalized axis removal. -/
theorem actualCartesianSource_integrableNormalizedFluxes
    ...
    (allGrades : ∀ index grade : ℕ, ∃ weighted : CoupledSpace (originalExhaustionRadius length index) length
      ...
      (∀ cell : ℤ,
        ...
        IntegrableOn (fun point => ‖point‖⁻¹ * ‖actualCartesianCovariantCell parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades cell point‖) openUnitDisk) ∧
  ...
  intro cell
  exact ⟨actualCartesianCofactorFluxCell_integrable_pair parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades
    M Mnonnegative stateBound vanishing native compatible cell,
    actualCartesianCovariantCell_integrable_pair parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades
      M Mnonnegative stateBound vanishing native compatible cell,
    actualCartesianXiCell_integrable_pair parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades
      M stateBound vanishing native compatible cell⟩
