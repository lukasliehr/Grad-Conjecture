...
include same in
/-- The actual original recovered vector gives the SAME full native covariant
core, before any derivative or Fourier coefficient is selected. -/
theorem actualRecoveredCovariant_polar (index : ℕ) (radius : ℝ)
    (inside : radius∈Icc (originalExhaustionRadius length index) 1) (angles : ℝ×ℝ) :
    coreValue (originalCovariantCore parameters length state.val.val.epsilon state.val.val.field core false)
  ...
  let curves := cartesianSourcePolarCurves parameters length compact lengthPositive widthHalf widthLength state small source flat
    fields member sameSources allGrades index
  have native := nativeCovariant_sameOriginalCore parameters length state.val.val.rho state.val.val.epsilon lengthPositive.ne'
    state.val.val.field
    ...
    lower positive bounded curves core radius inside angles
    (actualRecoveredVector_polar parameters length compact lengthPositive widthHalf widthLength state small source flat
      fields member sameSources allGrades compatible core same index radius inside angles.1 angles.2).symm
  rw [SmoothLowPhysicalRow.fullField_cartesianCovariant bounded curves radius inside angles] at native
  exact native.trans (actualNativeCovariantField_polar parameters length compact lengthPositive widthHalf widthLength state small source flat
    fields member sameSources allGrades compatible index radius inside angles.1 angles.2).symm
