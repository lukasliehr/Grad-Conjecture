import AKBW19ActualUniformRankTensorBudget
...
true covector principal tensor bounds at EVERY rank, for the SAME ledger.
The ball is fixed before the scale, state, source, cell power and rank. -/
theorem actualOriginalB10AllRankPrincipal (parameters : PhaseParameters)
          ...
          ∃ ledger : ActualLedger parameters admissible rho alpha delta parameter epsilon base,
            ...
            ∃ inverseCoherent : FamilyCoherent (determinantInverseFamily admissible ledger.val.gaugeDeviation),
              ...
              ∀ (rank : ℕ) (outer inner : Fin 2),
                ...
                ‖(StartupRankOperator.principalTensor admissible rank ledger.val ledger.property.1 inverseCoherent outer inner).fine‖ < tensorThreshold := by
  ...
  refine ⟨ledger, primitive, inverseCoherent, actualLedgerReduction_of_projection ledger inverseCoherent laws, rowsSmall, ?_⟩
