import AKBF3ActualGlobalOutputEnergy
...
/-- The actual pre-inverse-transpose Cartesian covariant, with all three
full integer-cell moments under the original analytic weight. -/
theorem actualCartesianCovariant_nativeCellMoments :
  ...
  obtain ⟨C,Cnonnegative,coefficientBound⟩ := covariantCurve_uniformBound parameters length compact state.val 2
  obtain ⟨constant,nonnegative,energy⟩ := actualOutputWeightedCurve_allGradeEnergy parameters length compact lengthPositive
    ...
    rows curves coherent 2 C Cnonnegative (fun index radius => coefficientBound (originalExhaustionRadius length index)
      (originalExhaustionRadius_positive length lengthPositive index)
  ...
  exact sameNativeCellMoments parameters (originalExhaustionRadius length)
    ...
    rows curves coherent constant nonnegative energy

/-- Xi/r is extracted directly from the SAME seventh-coordinate packet;
no singular multiplication of an unrelated scalar field is introduced. -/
theorem actualCartesianXiOverRadius_nativeCellMoments :
    ∃ constant : ℝ, 0 ≤ constant ∧
    ∃ field : Grad.CartesianStartup.StartupL2 1,
    ∃ moments : Fin 3 → Grad.CartesianStartup.StartupL2 1,
      moments 0 = field ∧
      (∀ᵐ point ∂volume.restrict openUnitDisk, ∀ cell,
        field point cell = cartesianWeight parameters cell point •
  ...
  obtain ⟨constant,nonnegative,energy⟩ := actualOutputWeightedCurve_allGradeEnergy parameters length compact lengthPositive
    widthHalf widthLength state small source flat fields member sameSources allGrades vanishing graded sameGrade constants estimate
    rows curves coherent 2 ‖nativeXiProjection parameters‖ (norm_nonneg _) (fun _ radius => xiCurve_uniformBound _ 2 radius)
  refine ⟨constant,nonnegative,?_⟩
  exact sameNativeCellMoments parameters (originalExhaustionRadius length)
    (originalExhaustionRadius_positive length lengthPositive)
    (fun index => (originalExhaustionRadius_half length lengthPositive index).trans_lt (by norm_num))
