import AKCH6ActualRecoveredDeterminantPolar
...
/-- SAME recovered original physical core satisfies the original determinant
forward row with the actual full source, including axis and outer boundary. -/
theorem actualRecoveredVector_determinant :
    quotientRowsDerivative parameters length 1 physicalState ![(0,core,scalar)] 2=source 2 := by
  ...
  have equation := actualRecoveredVector_determinant_polar parameters length compact lengthPositive widthHalf widthLength state small
    source flat fields member sameSources allGrades compatible core same physicalState sameBase sameEpsilon scalar index ‖point.val‖ inside (angle,axial)
  have divisionPoint : Grad.SourceCollarDivision.polarClosedPoint ‖point.val‖ angle
    ...
    exact polar
  change coreValue (quotientRowsDerivative parameters length 1 physicalState ![(0,core,scalar)] 2)
      (Grad.SourceCollarDivision.polarClosedPoint ‖point.val‖ angle
        ((originalExhaustionRadius_positive length lengthPositive index).le.trans inside.1.le) inside.2.le) axial 0=
    coreValue (source 2) (Grad.SourceCollarDivision.polarClosedPoint ‖point.val‖ angle
        ((originalExhaustionRadius_positive length lengthPositive index).le.trans inside.1.le) inside.2.le) axial 0 at equation
  rw [divisionPoint] at equation
  apply PiLp.ext
  intro coordinate
