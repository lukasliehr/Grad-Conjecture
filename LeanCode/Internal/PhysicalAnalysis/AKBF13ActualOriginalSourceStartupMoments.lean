...
import AKBF11ActualCoefficientOutputMoments
...
/-- Actual original higher-vanishing data supply the SAME covariant and Xi/r
startup moments. The checked fixed Q0, dilation, psi and K constructions apply directly. -/
theorem actualCartesianSource_startupNativeMoments
    ...
    (allGrades : ∀ index grade : ℕ, ∃ weighted : CoupledSpace (originalExhaustionRadius length index) length
      ...
      Nonempty (Grad.CartesianStartup.OriginalNativeMoments parameters
        (Grad.ActualSmoothPhysicalField.actualCartesianCovariantCell parameters length compact lengthPositive widthHalf widthLength state small
          source flat fields member sameSources allGrades)
        (Grad.ActualSmoothPhysicalField.actualCartesianXiOverRadiusCell parameters length compact lengthPositive widthHalf widthLength state small
          source flat fields member sameSources allGrades)) := by
  ...
  exact actualCartesianFamily_originalNativeMoments parameters length compact lengthPositive widthHalf widthLength state small source flat
