import AKCH3SameOriginalCovariantCore

noncomputable section
set_option autoImplicit false
set_option maxHeartbeats 1400000
open Set Filter MeasureTheory
open scoped Topology
namespace Grad.OriginalCoreRealization
open Grad.ClosedJets Grad.CartesianState Grad.Constraints Grad.AxisSplit Grad.Cor18
open Grad.NonlinearRange Grad.SourceCollar Grad.SourceCollarFullSource Grad.BoundaryTrace
open Grad.CartesianCoreRecovery Grad.ActualPuncturedFamily Grad.ActualAnnularExhaustion
open Grad.OriginalKernelHomogeneousGraph

/-- Faithfulness on the strict punctured disk; continuity supplies both the
axis and outer circle, using the existing original closed-jet realization. -/
theorem closedJet_zero_of_openPunctured {dimension : ℕ} (field : ClosedJet dimension)
    (zero : ∀ point : ClosedDisk,0<‖point.val‖ → ‖point.val‖<1 → field.value point=0) : field=0 := by
  apply closedJet_eq_of_value_eq
  apply continuousMap_eq_of_openDisk
  intro point membership
  change field.value point=0
  by_cases nonzero : point.val=0
  · have equality : point=originPoint := Subtype.ext nonzero
    rw [equality]
    change originValue field=0
    rw [← closedJetRay_zero field 0]
    have limit := (closedJetRay_continuous field 0).continuousAt.tendsto.comp
      (originalExhaustionRadius_tendsto 1)
    have zeros (index : ℕ) : closedJetRay field 0 (originalExhaustionRadius 1 index)=0 := by
      have positive := originalExhaustionRadius_positive 1 (by norm_num) index
      have bounded := (originalExhaustionRadius_half 1 (by norm_num) index).trans_lt (by norm_num : (1:ℝ)/2<1)
      have norm : ‖originalExhaustionRadius 1 index • spatialBasis 0‖=originalExhaustionRadius 1 index := by
        rw [norm_smul,Grad.BoundaryLift.spatialBasis_norm,mul_one,Real.norm_of_nonneg positive.le]
      let diskPoint : ClosedDisk := ⟨originalExhaustionRadius 1 index • spatialBasis 0,by change ‖_‖≤1; rw [norm]; exact bounded.le⟩
      exact (smoothClosedExtension_value field diskPoint).trans
        (zero diskPoint (by rw [norm]; exact positive) (by rw [norm]; exact bounded))
    have same : (fun index => closedJetRay field 0 (originalExhaustionRadius 1 index))=fun _ => 0 := funext zeros
    change Tendsto (fun index => closedJetRay field 0 (originalExhaustionRadius 1 index)) atTop (𝓝 (closedJetRay field 0 0)) at limit
    rw [same] at limit
    exact tendsto_nhds_unique limit tendsto_const_nhds
  · exact zero point (norm_pos_iff.mpr nonzero) membership

theorem originalCore_zero_of_openPuncturedValues {dimension : ℕ} (parameters : PhaseParameters)
    (field : ACore parameters dimension)
    (zero : ∀ point : ClosedDisk,0<‖point.val‖ → ‖point.val‖<1 → ∀ axial,coreValue field point axial=0) : field=0 := by
  apply Subtype.ext
  funext cell
  apply closedJet_zero_of_openPunctured
  intro point nonzero inside
  apply PiLp.ext
  intro coordinate
  exact coreValue_coordinate_zero parameters field point coordinate
    (fun axial => congrArg (fun value : ComplexEuclidean dimension => value coordinate) (zero point nonzero inside axial)) cell

theorem originalCore_eq_of_openPuncturedValues {dimension : ℕ} (parameters : PhaseParameters)
    (first second : ACore parameters dimension)
    (same : ∀ point : ClosedDisk,0<‖point.val‖ → ‖point.val‖<1 → ∀ axial,
      coreValue first point axial=coreValue second point axial) : first=second := by
  apply sub_eq_zero.mp
  apply originalCore_zero_of_openPuncturedValues parameters (first-second)
  intro point nonzero inside axial
  rw [Grad.GaugeCoefficients.Physical.RadialLedger.coreValue_subtract,same point nonzero inside axial,sub_self]

end Grad.OriginalCoreRealization
