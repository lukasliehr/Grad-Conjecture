import AKDP30ActualRowsOriginalRankControl
...
theorem actualPrincipal {State : Type*} {parameters : PhaseParameters} {L ell : ℝ}
    ...
    (force : StartupUniformOriginalRank parameters budget
    ...
    (third : StartupUniformOriginalRank parameters budget
    ...
    (flux : StartupUniformOriginalRank parameters budget
    ...
    (outer inner : Fin 2) :
    StartupUniformOriginalRank parameters budget
  ...
  have first := compFixed (StartupSpatialAction.value (L := L) (ell := ell) rank planarInclusionMap)
    (StartupSpatialAction.value_originalRankControlled parameters rank planarInclusionMap) force nonnegative
  have second := compFixed (StartupSpatialAction.value (L := L) (ell := ell) rank toroidalInclusionMap)
    (StartupSpatialAction.value_originalRankControlled parameters rank toroidalInclusionMap) third nonnegative
  have third := compFixed (StartupSpatialAction.value (L := L) (ell := ell) rank planarInclusionMap)
    (StartupSpatialAction.value_originalRankControlled parameters rank planarInclusionMap) flux nonnegative
  have first := compFixed (StartupSpatialAction.principalFixed (L := L) (ell := ell) rank outer inner 0)
    (StartupSpatialAction.principalFixed_originalRankControlled parameters rank outer inner 0) first nonnegative
  have second := compFixed (StartupSpatialAction.principalFixed (L := L) (ell := ell) rank outer inner 1)
    (StartupSpatialAction.principalFixed_originalRankControlled parameters rank outer inner 1) second nonnegative
  have third := compFixed (StartupSpatialAction.principalFixed (L := L) (ell := ell) rank outer inner 2)
    (StartupSpatialAction.principalFixed_originalRankControlled parameters rank outer inner 2) third nonnegative
