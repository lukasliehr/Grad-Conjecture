import AKCX56SameB10RankGaugeResolvent
...
/-- One fixed original B10 radius closes every spatial order, signed
axial power and natural cell weight. No PDE, coefficient identity, output
moment or unknown regularity is imposed as an extra hypothesis. -/
theorem actualOriginalNative_B10_allSpatialGrades
  ...
  obtain ⟨ledger,_primitive,inverseCoherent,_reduction,_rows,resolves,
  ...
  exact actualOriginalNative_allSpatialGrades parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades compatible
    ...
    outer inside outerSmooth insideSmooth outerCompact insideCompact outerDisk plateau radial resolves
