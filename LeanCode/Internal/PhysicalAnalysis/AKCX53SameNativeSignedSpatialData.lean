...
theorem startupNativeRows_sameSignedSpatialData (parameters : PhaseParameters) (L : ℝ) (scale : Scale) (lengthNonzero : L ≠ 0)
  ...
  · intro order regular direction
    exact StartupSignedFamily.HasSpatialGrade.nativeLowerFlux (detRegular order)
      (regular.value toroidalPartMap)
      (StartupSignedAction.actualScalarFlux_allSpatialGrade admissible data coherent inverseCoherent lengthNonzero scale.property.1.ne' order field regular)
      ((regular.value planarPartMap).recoveredGradient ((forceRegular order).sub
        (StartupSignedAction.actualForce_allSpatialGrade admissible data coherent inverseCoherent lengthNonzero scale.property.1.ne' order field regular))) direction
