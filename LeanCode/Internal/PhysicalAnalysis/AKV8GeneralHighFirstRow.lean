import AKV7GeneralActualWeightedSystem
...
def generalSharedHighXiRHS (mode : HighAnnularMode) : CollarL2 (ComplexEuclidean 1) lower :=
  sharedRawHighXiRHS parameters length compact lower positive lowerHalf lengthPositive widthHalf widthLength state small
    data mode

def generalSharedHighXRHS (mode : HighAnnularMode) : CollarL2 (ComplexEuclidean 1) lower :=
...
theorem generalSharedHighXiRHS_actual (mode : HighAnnularMode) :
    generalSharedHighXiRHS parameters length compact lower positive lowerHalf lengthPositive widthHalf widthLength state small data mode
      =ᵐ[volume.restrict (Icc lower 1)] fun radius =>
        (generalOriginalFullRHS parameters length compact lower positive lowerHalf lengthPositive state data
          (sharedStrongResponse parameters length compact lower positive lowerHalf lengthPositive widthHalf widthLength state small data) radius mode.val).2 := by
  ...
  simp only [generalOriginalFullRHS, actualOriginalUnknownRHS, generalOriginalSourceRHS,
