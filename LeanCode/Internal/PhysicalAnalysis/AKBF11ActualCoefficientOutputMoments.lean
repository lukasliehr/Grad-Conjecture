import AKBF9OriginalNativeMomentData
...
/-- Actual coefficient outputs retain full integer-cell moments after their
proved all-grade action. This interface does not commute frequency through convolution. -/
theorem actualCartesianFamily_outputMoments {dimension : ℕ}
    ...
    ∃ family : Grad.CartesianStartup.StartupMoments dimension,
      ∀ᵐ point ∂volume.restrict openUnitDisk, ∀ cell : ℤ,
        family.field point cell = cartesianWeight parameters cell point •
          gluedCartesianCellField parameters (originalExhaustionRadius length)
            ...
            (originalExhaustionRadius_tendsto length) rows curves cell point := by
  obtain ⟨constant,nonnegative,energy⟩ := actualOutputWeightedCurve_allGradeEnergy parameters length compact lengthPositive
    ...
    rows curves coherent 2 C Cnonnegative coefficientBound
  obtain ⟨field,moments,zero,same,grades,_⟩ := sameNativeCellMoments parameters (originalExhaustionRadius length)
    (originalExhaustionRadius_positive length lengthPositive)
    (fun index => (originalExhaustionRadius_half length lengthPositive index).trans_lt (by norm_num))
  ...
  exact ⟨⟨field,moments,zero,grades⟩,same⟩
