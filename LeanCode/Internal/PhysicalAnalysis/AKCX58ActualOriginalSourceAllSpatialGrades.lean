import AKCX57ActualNativeB10AllSpatialGrades
...
/-- Genuine original source consumer of closed native all-order regularity. The compatible
family, all natural native moments, exact weak ER identities, all signed
source all-order graphs and punctured complement graphs are actually supplied.
Every spatial order, signed axial power and natural weight of the SAME weighted Q0(a_C) has its genuine global graph. -/
theorem actualCartesianSource_allSpatialGrades
    ...
    (outerDisk : tsupport outer ⊆ openUnitDisk)
    ...
    (low : physicalBudget parameters state.val.val.field state.val.val.rho state.val.val.epsilon 10 <
      actualNativeSpatialRadius parameters length compact lengthPositive state.val.val.compactNonnegative
    ...
    (allGrades : ∀ index grade : ℕ, ∃ weighted : CoupledSpace (originalExhaustionRadius length index) length
      ...
      (∃ weighted : StartupAllMoments 3,
        ...
        ∀ order, ((StartupSignedFamily.ofNatural weighted length (originalStartupScale length lengthPositive).val).circle).HasSpatialGrade order) := by
  ...
  exact actualOriginalNative_B10_allSpatialGrades parameters length compact lengthPositive widthHalf widthLength state small source flat fields member sameSources allGrades laws.2.1
    ...
    outer inside outerSmooth insideSmooth outerCompact insideCompact outerDisk plateau radial low
