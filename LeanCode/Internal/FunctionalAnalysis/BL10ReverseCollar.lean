...
def polarJetEnvelope {Value : Type*} [NormedAddCommGroup Value] [NormedSpace ℝ Value]
    (field : ℝ × ℝ → Value) (grade : ℕ) (point : ℝ × ℝ) : ℝ :=
  ∑ order ∈ Finset.range (grade + 1), ‖iteratedFDeriv ℝ order field point‖

theorem polarJetEnvelope_bound {Value : Type*} [NormedAddCommGroup Value] [NormedSpace ℝ Value]
    (field : ℝ × ℝ → Value) (grade order : ℕ) (upper : order ≤ grade) (point : ℝ × ℝ) :
    ‖iteratedFDeriv ℝ order field point‖ ≤ polarJetEnvelope field grade point := by
...
theorem reverseCollar_axis_derivative_bound {Value : Type*} [NormedAddCommGroup Value]
    ...
    (grade order : ℕ) (upper : order ≤ grade) (radius : ℝ) (inside : radius ∈ Icc (3 / 4 : ℝ) 1) :
