...
def polarRadialComponentLinear (angle : ℝ) : ComplexEuclidean 2 →L[ℂ] ℂ :=
  (Real.cos angle : ℂ) • PiLp.proj 2 (fun _ : Fin 2 => ℂ) 0 +
    (Real.sin angle : ℂ) • PiLp.proj 2 (fun _ : Fin 2 => ℂ) 1

@[simp] theorem polarRadialComponentLinear_apply (angle : ℝ)
    (value : ComplexEuclidean 2) :
    polarRadialComponentLinear angle value = polarRadialComponent angle value := by
  simp [polarRadialComponentLinear, polarRadialComponent]

theorem radialSourceCore_axis_value {parameters : PhaseParameters}
    (field : ACore parameters 2) (cell : ℤ)
    (radius : ℝ) (bounded : |radius| ≤ 1) :
    ((radialSourceCore parameters field).val cell).value
        (axisClosedPoint radius bounded) 0 =
      polarRadialMean (field.val cell) radius bounded 0 := by
  rw [radialSourceCore_eq_average_sub_tangential]
  change (((equivariantAverageCore parameters field).val cell -
    (tangentialCore parameters field).val cell).value
      (axisClosedPoint radius bounded)) 0 = _
  ...
  rw [equivariantAverageCore_apply, tangentialCore_apply,
    equivariantAverage_axis_radial]
  have axisEquality : polarClosedPoint radius bounded 0 = axisClosedPoint radius bounded := by
    apply Subtype.ext
  ...
  rw [← axisEquality,
    tangentialJet_polar_axis_value (field.val cell) radius bounded 0]
  simp [axisClosedPoint, polarTangentialVector]

theorem polarRadialComponent_norm_le_two (angle : ℝ)
    (value : ComplexEuclidean 2) :
    ‖polarRadialComponent angle value‖ ≤ 2 * ‖value‖ := by
  unfold polarRadialComponent
  calc
    _ ≤ ‖(Real.cos angle : ℂ) * value 0‖ +
        ‖(Real.sin angle : ℂ) * value 1‖ := norm_add_le _ _
    _ = |Real.cos angle| * ‖value 0‖ + |Real.sin angle| * ‖value 1‖ := by
      simp only [norm_mul, Complex.norm_real, Real.norm_eq_abs]
    _ ≤ 1 * ‖value‖ + 1 * ‖value‖ := by
      exact add_le_add
        (mul_le_mul (Real.abs_cos_le_one angle) (PiLp.norm_apply_le value 0)
          (norm_nonneg _) zero_le_one)
        (mul_le_mul (Real.abs_sin_le_one angle) (PiLp.norm_apply_le value 1)
          (norm_nonneg _) zero_le_one)
    _ = 2 * ‖value‖ := by ring

/-- The accepted Cartesian radial constraint is precisely zero mean of the
physical radial component on every polar circle, after full cell-Fourier
evaluation. -/
theorem sourceAngularAverage_polarRadial
    {parameters : PhaseParameters} (field : ACore parameters 2)
    (radius : ℝ) (bounded : |radius| ≤ 1) (axialAngle : ℝ) :
    sourceAngularAverage (fun polarAngle => polarRadialComponent polarAngle
      (sourceCoreValue field (polarClosedPoint radius bounded polarAngle) axialAngle)) =
      (coreValue (radialSourceCore parameters field)
        (axisClosedPoint radius bounded) axialAngle) 0 := by
  let term : ℤ → ℝ → ℂ := fun cell angle =>
    axialPhase cell axialAngle * polarRadialComponent angle
  ...
  have law : HasSum (fun cell => ∫ angle in (0 : ℝ)..2 * Real.pi, term cell angle)
    ...
    · intro cell
      ...
      exact (polarRadialComponent_norm_le_two angle _).trans
    ...
    · filter_upwards with angle _
      ...
      have mapped := (polarRadialComponentLinear angle).map_tsum vectorSummable
      ...
      rw [← show (∑' cell : ℤ, term cell angle) =
        ...
        change (∑' cell : ℤ, axialPhase cell axialAngle *
          polarRadialComponent angle ((field.val cell).value
            (polarClosedPoint radius bounded angle))) = _
        simpa only [sourceCoreValue, coreValue, map_smul, smul_eq_mul,
          polarRadialComponentLinear_apply] using mapped.symm]
  ...
  have termLaw (cell : ℤ) :
      (2 * Real.pi)⁻¹ • (∫ angle in (0 : ℝ)..2 * Real.pi, term cell angle) =
        axialPhase cell axialAngle •
          (((radialSourceCore parameters field).val cell).value
            (axisClosedPoint radius bounded) 0) := by
    dsimp only [term]
    rw [intervalIntegral.integral_const_mul]
    rw [radialSourceCore_axis_value]
  ...
  have sumEquality :
      (∑' cell : ℤ, axialPhase cell axialAngle •
        ...
        (coreValue (radialSourceCore parameters field)
          (axisClosedPoint radius bounded) axialAngle) 0 := by
    have mapped := (PiLp.proj (𝕜 := ℂ) 2 (fun _ : Fin 2 => ℂ) 0).map_tsum
      (coreValue_summable (radialSourceCore parameters field)
    ...
    change ((∑' cell : ℤ, axialPhase cell axialAngle •
      ...
      ∑' cell : ℤ, axialPhase cell axialAngle *
        (((radialSourceCore parameters field).val cell).value
          (axisClosedPoint radius bounded) 0) at mapped
...
theorem actualCartesianSourceSlice_F1_mean_zero
    {parameters : PhaseParameters} (source : CartesianSourceCore parameters)
    (flat : CartesianCoreIsFlat source)
    (radius : ℝ) (bounded : |radius| ≤ 1) (axialAngle : ℝ) :
    sourceAngularAverage (fun angle =>
      polarRadialComponent angle
        ((actualCartesianSourceSlice source radius bounded axialAngle).planar angle)) = 0 := by
  change sourceAngularAverage (fun angle =>
    polarRadialComponent angle
      (sourceCoreValue source.1 (polarClosedPoint radius bounded angle) axialAngle)) = 0
  rw [sourceAngularAverage_polarRadial source.1 radius bounded axialAngle, flat.1]
  simp [coreValue]
